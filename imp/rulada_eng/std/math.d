 ...
 *  )
 */
real acos(real x)               { return std.c.acosl(x); }
 ...
 *  )
 */
real asin(real x)               { return std.c.asinl(x); }
 ...
 *  )
 */
real atan(real x)               { return std.c.atanl(x); }
 ...
 *      )
 */
real atan2(real y, real x)      { return std.c.atan2l(y,x); }
 ...
 *
 * If x is not a special value, the result то же самое, что
 * $(D cast(int)logb(x)).
 *
 *      $(TABLE_SV
 ...
 *      )
 */
int ilogb(real x)               { return std.c.ilogbl(x); }

alias std.c.FP_ILOGB0   FP_ILOGB0;
alias std.c.FP_ILOGBNAN FP_ILOGBNAN;
...
real log1p(real x)              { return std.c.log1pl(x); }
 ...
 *      )
 */
real logb(real x)               { return std.c.logbl(x); }
 ...
 * )
 */
real modf(real x, inout real y) { return std.c.modfl(x,&y); }
 ...
 *      )
 */
real cbrt(real x)               { return std.c.cbrtl(x); }
 ...
 */
real erf(real x)                { return std.c.erfl(x); }
 ...
 */
real erfc(real x)               { return std.c.erfcl(x); }
 ...
 */
real ceil(real x)               { return std.c.ceill(x); }

/**************************************
 * Returns the value of x rounded downward to the next integer
 * (toward negative infinity).
 */
real floor(real x)              { return std.c.floorl(x); }
 ...
 */
real nearbyint(real x) { return std.c.nearbyintl(x); }
 ...
 * the even integer.
 */
real round(real x) { return std.c.roundl(x); }
 ...
 */
real trunc(real x) { return std.c.truncl(x); }
 ...
 */
real remainder(real x, real y) { return std.c.remainderl(x, y); }

real remquo(real x, real y, out int n)  /// ditto
{
    version (Posix)
        return std.c.remquol(x, y, &n);
    else
 ...
 */
real nan(char[] tagp) { return std.c.nanl(std.string.toStringz(tagp)); }
 ...
 *
 * Возвращает: the number of mantissa bits which are equal in x and y.
