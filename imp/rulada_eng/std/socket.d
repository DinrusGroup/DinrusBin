 ...
 * Пример: See /dmd/samples/d/listener.d.
...
private import std.c: uint32_t, uint16_t, int32_t;
...
version(Win32)
{
	...
	private alias os.windows.timeval _ctimeval;

	typedef SOCKET socket_t = INVALID_SOCKET;
	private const int _SOCKET_ERROR = SOCKET_ERROR;
    alias socket_t т_сокет;
...
else version(BsdSockets)
{
	version(Posix)
	{
	    private import os.pos.socket;
	    private import os.posix;
	    private alias os.posix.timeval _ctimeval;
	}
...
class SocketException: Error
{
	int errorCode; /// Platform-specific error code.
	alias errorCode кодОш;
...
alias SocketException СокетИскл;
...
alias AddressFamily СемействоАдресов;
...
enum SocketType: int
{
	...
	НЕОБР = SOCK_RAW,
	НПС = SOCK_RDM ,		
	ПОСЛЕДПАК =SOCK_SEQPACKET,
	}

alias SocketType ТипСокета;
...
alias ProtocolType ТипПротокола;
...
class Protocol
{
	...
	alias aliases псевдонимы;


	void populate(protoent* proto);
	alias populate  наполни;

	/** Returns false on failure */
	bool getProtocolByName(string name);	
	alias getProtocolByName дайПротоколПоИмени;

	/** Returns false on failure */
	// Same as getprotobynumber().
	bool getProtocolByType(ProtocolType type);
	alias getProtocolByType дайПротоколПоТипу;
}

alias Protocol Протокол;
...
class Service
{
	...
	alias protocolName имяПротокола;
	...
	alias getServiceByName дайСлужбуПоИмени;
	...
	alias getServiceByPort дайСлужбуПоПорту;
}
alias Service Служба;
...
class InternetHost
{
	...
	alias getHostByName дайХостПоИмени;
	/**
	...
	alias getHostByAddr дайХостПоАдр;
}
alias InternetHost ИнтернетХост;
...
alias AddressException ИсклАдреса;
...
{
alias name имя;
alias nameLen длинаИм;
alias addressFamily семьяАдресов;
 ...
 *
 */
 alias UnknownAddress НезнакомыйАдрес;
 ...
 * 4) address and port.
 */
 alias InternetAddress АдресИнтернета;
...
alias toString вТкст;
alias parse разбор;
alias toPortString вТкстПорта;
alias port порт;
alias addr адр;
alias toAddrString вТкстАдр;
	...
	const uint АДР_ЛЮБОЙ = INADDR_ANY;
	const uint АДР_НУ = INADDR_NONE;
	const ushort ПОРТ_ЛЮБОЙ = 0;
...
alias SocketAcceptException ИсклПриёмаСокета;
...
alias SocketShutdown ЭкстрЗакрытиеСокета;
...
alias SocketFlags ФлагиСокета;

/// Duration timeout value.
extern(C) struct timeval
{
alias seconds сукунды;
alias microseconds микросекунды;
...
alias add добавь;
alias remove удали;
alias isSet набор_ли;
alias max макс;
alias toFd_set вФд_набор;
alias selectn выберином;
...
alias SocketOptionLevel УровеньОпцииСокета;
...
class Socket
{
	 ...
	 * be received.
	 * Возвращает: the number of bytes actually received,
	...
	body
	{
		...
		{
			if(_SOCKET_ERROR == result && std.c.getErrno() == EINTR)
				return -1;
		}
		else
		{
