...
version(gtk)
{
	public import rae.gl.GLExtensions;
}
...
private void* getProc (char[] procname)
{
	...
	try
	{
		symbol = glextdrv.getSymbol( stringz.toStringz(procname) );
	}
	catch(SharedLibException)
	{
		debug(OpenGLLoader) Trace.formatln("Failed to load OpenGL (ext) proc address " ~ procname ~ ".");
	}
	return symbol;
}
...
private void* getProc (char[] procname) {
	...
	if (symbol is null) {
		//printf (("Failed to load OpenGL (ext) proc address " ~ procname ~ ".\n\0").ptr);
	}
	return symbol;
}
...
version(Rulada)
{
	version (Windows)
	{
		glextdrv = SharedLib.load("OpenGL32.dll");
	}
	else version (linux)
	{
		glextdrv = SharedLib.load("libGL.so");
	}
	else version (darwin)
	{
		try
		{
			//glextdrv = SharedLib.load("/System/Library/Frameworks/OpenGL.framework/Libraries/libGL.dylib");
			glextdrv = SharedLib.load("/System/Library/Frameworks/OpenGL.framework/OpenGL/");
		}
		catch(SharedLibException)
		{
...
else //Phobos
{
	version (Windows) {
		...
		glextdrv = ExeModule_Load("/System/Library/Frameworks/OpenGL.framework/Libraries/libGL.dylib");
	}
}
