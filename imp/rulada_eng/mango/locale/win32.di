module mango.locale.win32;

alias mango.locale.win32 nativeMethods;
...
int compareString(int lcid, char[] stringA, uint offsetA, uint lengthA, char[] stringB, uint offsetB, uint lengthB, bool ignoreCase) {
  ...
  wchar[] toUnicode(char[] string, uint offset, uint length, out int translated) {
    ...
    translated = MultiByteToWideChar(0, 0, chars, length, cast(wchar*) result, required);
    return result;
  }
  ...
  return CompareStringW(sortId, ignoreCase ? 0x1 : 0x0, cast(wchar*) string1, len1, cast(wchar*) string2, len2) - 2;
}
version (build) {
    debug {
        pragma(link, "mango");
