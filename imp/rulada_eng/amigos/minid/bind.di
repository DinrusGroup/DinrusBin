...
alias amigos.minid.utils.isArrayType isArrayType;
...
public word getWrappedClass(MDThread* t, TypeInfo ti)
{
	...
	{
		getRegistryVar(t, "amigos.minid.bind.WrappedClasses");
		pushNativeObj(t, ti);
...
{
	getRegistryVar(t, "amigos.minid.bind.WrappedInstances");
	pushNativeObj(t, o);
	idx(t, -2);
	deref(t, -1);
...
public word superPush(Type)(MDThread* t, Type val)
{
	...
	else static if(tango.core.Traits.isStringType!(T))
	{
		static if(is(T : char[]))
...
public Type superGet(Type)(MDThread* t, word idx)
{
	...
	static if(!tango.core.Traits.isStringType!(T) && isArrayType!(T))
	{
...
public bool canCastTo(Type)(MDThread* t, word idx)
{
	...
	else static if(tango.core.Traits.isStringType!(T) || is(T : MDString))
	{
...
{
	getRegistry(t);
	pushString(t, "amigos.minid.bind.initialized");

	if(!opin(t, -1, -2))
	{
		newTable(t);       fielda(t, -3, "amigos.minid.bind.WrappedClasses");
		newTable(t);       fielda(t, -3, "amigos.minid.bind.WrappedInstances");
