...
private import rt.gc.gcbits;
//private import gcstats;

private import cstdlib = std.c : calloc, free, malloc, realloc;
private import cstring = std.c : memcpy, memmove, memset;

debug private import std.c;
...
version (Win32)
{
    import rt.gc.gcalloc;
    import rt.core.os.windows;
}

version (OSX)
{
    import rt.core.os.osx;
}
...
class GC
{
    ...
    final void enable()
    {
        if (!thread_needLock()||std.thread.Thread.nthreads == 1)
        {
    ...
    final void disable()
    {
        if(!thread_needLock()||std.thread.DThread.nthreads == 1)
        {
            gcx.disabled++;
        }
        else synchronized (gcLock)
    ...
    private void *mallocNoSync(size_t size, uint bits = 0)
    {
        ...
        {
		debug(PRINTF) printf("  from mallocNS to bigAlloc\n");
            p = gcx.bigAlloc(size);
			debug(PRINTF) printf("past bigAlloc p: %i size: %i", p, size);
            if (!p)
                onOutOfMemoryError();
        }
        size -= SENTINEL_EXTRA;
		...
		debug(PRINTF) printf("  return from mallocNS %i\n", p);
        return p;
    }
    ...
    final void *calloc(size_t size, uint bits = 0)
    {
        ...
        synchronized (gcLock)
        {
		debug(PRINTF) printf(" calloc returning");
...
/*
  ...
  private size_t sizeOfNoSync(void *p)
	...
	else
	{
	    ...
	    {
		gcx.cached_size_key = p;
		gcx.cached_size_val = size;
	    }
...
*/
    ...
    {
	debug(PRINTF) printf("addR1: void*, size_t");
        if (!p || !sz)
        {
            return;
        }
...
void addRange(void *pbot, void *ptop)
    {
	debug(PRINTF) printf(" addR2: void*, void*");
        if (!pbot || !ptop)
        {
            return;
        }
...
//////////////////////////////////////////////////
    ...
    void *bigAlloc(size_t size)
    {
      ...
      L1:
        ...
        //debug(PRINTF)		printf("bigAlloc: \tp = %x\n", p);
        return p;
