// Написано на языке программирования Ди

/**
 * Привязка к функции аргументов.
 *
 * Ссылки:
 *	$(LINK2 http://www.boost.org/libs/bind/bind.html, boost::bind)	
 * Авторы: Томаш Стаховяк
 * Дата: Ноябрь 28, 2006
 * Макросы:
 *	WIKI = Phobos/StdBind
 * Авторское Право:
...
/**
 Переданные функции 'bind', они будут отмечать динамические параметры - те, которые не  могут быть привязаны статически
	В boost они называются __1, __2, __3, и т.д..., а здесь __0, __1, __2, ...
*/
const DynArg!(0) _0;
...
/*
	Определить, является ли данный тип DynArg с любым индексом
*/
template isDynArg(T) {
	static if (is(typeof(T.argNr))) {				// должно иметься поле  argNr
		static if(is(T : DynArg!(T.argNr))) {		// теперь точная проверка типа
...
/*
	Определить, является ли данный тип DynArg с заданным индексом
*/
template isDynArg(T, int i) {
	static const bool isDynArg = is(T : DynArg!(i));
}

alias isDynArg динАрг_ли;
/*
	Преобразует множество статического типа в динамический тип
*/
template DynamicArrayType(T) {
	alias typeof(T[0])[] DynamicArrayType;
}

alias DynamicArrayType ТипДинамМас;
/*
	Присваивает одну сущность другой. Так как все статические массивы не любят нормальных присваиваний, то к ним применяются присваивания среза.
...
/*
	...
	Как правило, используется только неявное преобразование, но когда оба операнда числового типа, то над ними выполняется явное приведение к типу.
	...
	Параметры:
		...
		Y = исходный тип
		b = источник
		copyStaticArrays = когда статический массив присваивается динамическому, иногда его нужно .dup'ить, так как хранилище может размещаться в непостоянных (volatile) местах.
*/
template _assign(T, Y, bool copyStaticArrays = true) {
	static if (isStaticArray!(T)) {
		
		// Если приемник статический массив, то каждый элемент из источника копируется в него с помощью foreach
		...
		// приемником является динамический массив, а источником статический. Это порой требует .dup
...
struct Tuple() {
	...
	alias EmptyTuple_ ПустойКортеж_;
...
/**
	Создать динамически кортеж из заданных элементов
*/
Tuple!(T) tuple(T ...)(T t) {
	Tuple!(T) res;
	foreach (i, x; t) {
...
/**
	Проверяет, является ли данный тип структурой Tuple любой длины
*/
...
alias isTypeTuple типКортеж_ли;
...
alias minNumArgs_impl минЧлоАргов_реализ;
...
/**
	Находит минимальное число аргументов, которое следует предоставить данной функции
*/
alias minNumArgs минЧлоАргов;
...
alias MBoundFunc МПривязанФунк;
template MBoundFunc() {
	// meta
	alias FAlias_	FAlias;
	alias FT		FuncType;
	alias AllBoundArgs_	AllBoundArgs;
	// all arguments given to bind() or bindAlias()
	
	static if (!is(typeof(FAlias) == EmptySlot)) {
		alias Tuple!(ParameterTypeTuple!(FT))	RealFuncParams;
		// the parameters of the bound function
		alias FuncReferenceParamsAsPointers!(FAlias)	FuncParams;
		// references converted to pointers
	} else {
		alias Tuple!(ParameterTypeTuple!(FT))	FuncParams;
		// the parameters of the bound function
	}
	...
	alias ExtractedBoundArgs!(AllBoundArgs.type)	BoundArgs;	
	// 'saved' arguments. this includes nested/composed functions
