/**
 * Данный модуль - это набор из типов и функций, предназначенный для преобразования любого объекта (значения
 * или кучи) в жанровый тип box, который позволяет пользователю передать данный объект, не
 * зная того, что находится в боксе, а затем позволяет восстановить данное значение. 
 *
 * Пример:
---
// Преобразовать целое 45 в box.
Box b = box(45);

// Восстановить целое и привести его в real.
real r = unbox!(real)(b);
---
 *
 * Это интерфейс-основа, и как правило, больше ничего понимать не надо.
 * Если не удается с его помощью распаковать объект в определенный тип, то выводится
 * UnboxException. Как показано далее, для подражания поведению статических типов
 * используются неявные приведения к типу.
 * Так, например, можно распаковать из int в
 * real, но нельзя сделать этого из real в int: для этого потребуется явное
 *приведение к типу. 
 *
 * Следовательно, подразумевается, что попытка распаковки int в виде string выведет
 * вместо его форматирования ошибку. В целом, можно использовать метод toString
 * в отношении box и получить правильный результат, в зависимости от того, принимает ли std.string.format
 * это. 
 *
 * Боксы можно сравнивать друг с другом и их можно использовать в качестве ключей к
 * ассоциативным массивам. 
 *
 * Есть также функции для преобразования в или из боксов.
 *
 * Пример:
---
// Преобразовать аргумент в массив боксов.
Box[] a = boxArray(1, 45.4, "foobar");

// Преобразовать массив боксов в аргументы.
TypeInfo[] arg_types;
void* arg_data;

boxArrayToArguments(a, arg_types, arg_data);

// Преобразовать аргументы обратно в боксы, используя
// другую форму функции.
a = boxArray(arg_types, arg_data);
---
 * Одно из применений боксинга - для более легкой и мощной поддержки вариадической функции;
 * просто вызывается "boxArray(_arguments, _argptr)", а затем над массивом производится
 * всё, что угодно.
 *
 * Авторы:
 *	Burton Radons
...
/** Вернуть следующий тип в массиве typeinfo, или null, если так ничего нет. */
...
alias isArrayTypeInfo массивТипИнфо_ли;

/** Класс-тип, возвращенный от Box.findTypeClass; важен порядок записей. */
...
private enum TypeClass
{
	...
	//alias Float ПлавТочка;
	
    Complex, /**< cfloat, cdouble, creal */
	//alias Complex Комплексное;
	
    Imaginary, /**< ifloat, idouble, ireal */
	//alias Imaginary Воображаемое;
	
    Class, /**< Inherits from Object */
	//alias Class Класс;
	
    Pointer, /**< Pointer type (T *) */
	//alias Pointer Указатель;
...
private enum КлассТипа
{
    Булев, /**< бул */	
    Бит = Булев,	// for backwards compatibility		
    Целое, /**< байт, ббайт, крат, бкрат, цел, бцел, дол, бдол */
	ПлавТочка, /**< плав, дво, реал */
	Комплекс, /**< кплав, кдво, креал */
	Воображаемое, /**< вплав, вдво, вреал */
	Класс, /**< Наследует от Объект */
	Указатель, /**< тип Указатель (T *) */
	Массив, /**< Array type (T []) */
	Другой, /**< Any other type, such as delegates, function pointers, struct, void... */
	
}
/**
 * Box - генерный контейнер для объектов (и значения, и кучи), позволяющий
 * пользователю помещать их в генерную форму и восстанавливать позднее.
 * Бокс-объект содержит значение в генерном виде, позволяя передавать его
 * из одного места в другое, не зная его типа.  Он создается вызовом
 * функции box, а восстановить значение можно создав экземпляр
 * шаблона unbox.
 */
 alias Box Бокс;
...
struct Box
{
 ...
 alias findTypeClass найтиКлассТип;
	
    /** Вернуть, можно ли разбоксить данное значение, без throwing. */
...
alias argumentLength длинаАргумента;
/**
 ...
 *
 * Выводит исключение: UnboxException if it cannot cast
 *
 * Пример:
 * ---
 * Box b = box(4.5);
...
//DLL
 ...
 UnboxException OnUnboxException(Box obj, TypeInfo outputType){
	throw new UnboxException(obj, outputType);
	}