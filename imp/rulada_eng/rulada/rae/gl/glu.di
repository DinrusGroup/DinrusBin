...
private void* getProc (char[] procname)
{
	...
	try
	{
		symbol = gludrv.getSymbol( stringz.toStringz(procname) );
	}
	catch(SharedLibException)
	{
		debug(OpenGLLoader) Trace.formatln("Failed to load GLU proc address " ~ procname ~ ".");
	}
	return symbol;
}
...
version(Rulada)
{
	version (Windows)
	{
		gludrv = SharedLib.load("glu32.dll");
	}
	else version (linux)
	{
		gludrv = SharedLib.load("libGLU.so");
	}
	else version (darwin)
	{
		try
		{
			//gludrv = SharedLib.load("/System/Library/Frameworks/OpenGL.framework/Libraries/libGLU.dylib");
			gludrv = SharedLib.load("/System/Library/Frameworks/OpenGL.framework/OpenGL");
		}
		catch(SharedLibException)
		{
			throw new Exception("Library load failed: /System/Library/Frameworks/OpenGL.framework/Libraries/libGLU.dylib");
		}
	}
}
else //Phobos
{
	version(Windows) {
		...
		gludrv = ExeModule_Load("/System/Library/Frameworks/OpenGL.framework/Libraries/libGLU.dylib");
	}
}
