...
private template hasObjects(T...)
{
    ...
    {
        const hasObjects = hasObjects!(U, T[1 .. $]);
    }
    ...
    {
        const hasObjects = is(T[0] == class) || hasObjects!(T[1 .. $]);
    }
}
...
{
    const hasAliasing = hasRawAliasing!(T) || hasObjects!(T);
}
...
{
    static if (T.min == 0) const byte mostNegative = 0;
    else static if (T.min > 0) const mostNegative = -T.max;
    else const mostNegative = T.min;
}
