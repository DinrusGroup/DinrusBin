 ...
 * atomically updated <tt>int</tt> value manipulated using the getter
 * and setter for the state property and compareAndSet32 is tracked
 * with respect to synchronization.
 *
 * <p>Subclasses should be defined as non-public internal helper
 * classes that are used to implement the synchronization properties
 * of their enclosing class.  Class <tt>AbstractLock</tt> does not
 ...
 * mode.
 *
 * <p>This class defines a nested ConditionObject class that can be
 ...
 * <tt>AbstractLock</tt> method otherwise creates such a condition, so
 ...
 * using an <tt>AbstractLock</tt> for their synchronization mechanics.
 *
 * <h3>Usage</h3>
 *
 ...
 * <tt>false</tt> only if hasQueuedThreads returns <tt>true</tt> and
 * <tt>getFirstQueuedThread</tt> is not the current thread; or
 * equivalently, that <tt>getFirstQueuedThread</tt> is both non-null
 ...
 * \endcode
 *
 * <p> Here is a latch class that is like a CountDownLatch except that
 * it only requires a single <tt>notify</tt> to fire. Because a latch
 * is non-exclusive, it uses the <tt>shared</tt> acquire and release
 * methods.
 *
 * \code
 * class BoolLatch {
...
abstract class AbstractLock {
  ...
  /**
   * Creates a new <tt>AbstractLock</tt> instance
   ...
   *
   * <p>CLH queues need a dummy header node to get started. But we
   ...
   * contention.
   *
   * <p>Threads waiting on Conditions use the same nodes, but use an
  ...
  final static class Node {
     ...
     *               on failure, block.
     *   CANCELLED:  Node is cancelled due to timeout
     ...
     */
    int waitStatus;
    int v_waitStatus() { volatile return waitStatus; }
    void v_waitStatus(int val) { volatile waitStatus = val; }
     ...
     */
    Node prev;
    Node v_prev() { volatile return prev; }
    void v_prev(Node val) { volatile prev = val; }
     ...
     * node if cancelled.  The enq operation does not assign next
     * field of a predecessor until after attachment, so seeing a null
     * next field does not necessarily mean that node is at end of
     * queue. However, if a next field appears to be null, we can scan
     * prev's from the tail to double-check.
     */
    Node next;
    Node v_next() { volatile return next; }
    void v_next(Node val) { volatile next = val; }
     ...
     */
    Thread thread;
    Thread v_thread() { volatile return thread; }
    void v_thread(Thread val) { volatile thread = val; }
    ...
    final Node predecessor() {
      volatile return prev;
    }

    this() {    // Used to establish initial head or SHARED marker
    }

    this(Thread thread, Node mode) {     // Used by addWaiter
      this.nextWaiter = mode;
      this.thread = thread;
    }

    this(Thread thread, int waitStatus) { // Used by Condition
   ...
   * initialization, it is modified only via method setHead.  Note: If
   * head exists, its waitStatus is guaranteed not to be CANCELLED.
   */
  private Node head;
  Node v_head() { volatile return head; }
  void v_head(Node val) { volatile head = val; }
   ...
   */
  private Node tail; 
  Node v_tail() { volatile return tail; }
  void v_tail(Node val) { volatile tail = val; }
  ...
  private Node enq(Node node) {
    for (;;) {
      Node t = v_tail;
      if (t is null) { // Must initialize
	Node h = new Node(); // Dummy header
	volatile {
	  h.next = node;
	  node.prev = h;
	}
	if (Atomic.compareAndSet32(cast(void*) &head, null, cast(void*) h)) {
	  v_tail = node;
	  return h;
	}
      }
	...
	node.v_prev = t;     
	if (Atomic.compareAndSet32(cast(void*) &tail,cast(void*) t, cast(void*) node)) {
	  t.v_next = node; 
	  return t; 
	}
      }
  ...
  private Node addWaiter(Node mode) {
    Node node = new Node(Thread.getThis(), mode);
    // Try the fast path of enq; backup to full enq on failure
    Node pred = v_tail;
    if (pred !is null) {
      node.v_prev = pred;     
      if (Atomic.compareAndSet32(cast(void*) &tail,cast(void*) pred, cast(void*) node)) {
	pred.v_next = node; 
	return node;
      }
    }
  ...
  private void unparkSuccessor(Node node) {
     ...
     */
    Atomic.compareAndSet32(&node.waitStatus, Node.SIGNAL, 0);
        
    /*
     * Thread to unpark is held in successor, which is normally just
     * the next node.  But if cancelled or apparently null, traverse
	...
	thread = null;
	for (s = tail; s !is null && s !is node; ) {
	  volatile {
	    if (s.waitStatus <= 0)
	      thread = s.thread;
	    s = s.prev;
  ...
  private void setHeadAndPropagate(Node node, int propagate) {
    setHead(node);
    if (propagate > 0 && node.v_waitStatus != 0) {
      /*
  ...
  private void cancelAcquire(Node node) {
    if (node !is null) { // Ignore if node doesn't exist
      volatile {
	node.thread = null;
  ...
  private static bool shouldParkAfterFailedAcquire(Node pred, Node node) {
    ...
    if (s > 0) 
       ...
       * parking.
       */
      Atomic.compareAndSet32(&pred.waitStatus, 0, Node.SIGNAL);
    }
    return false;
  }
  ...
  final void acquireQueued(Node node, int arg) {
	...
	if (p is v_head && tryAcquire(arg)) {
	  setHead(node);
	  p.v_next = null; // help GC
	  return;
	}
...
version (Ares)
  ...
  private bool doAcquireNanos(int arg, long nanosTimeout) {
    long lastTime = currentTimeNanos();
...
version (Ares)
  ...
  final Thread getFirstQueuedThread() {
    ...
    volatile return (head is tail)? null : fullGetFirstQueuedThread();
  }
  ...
  private Thread fullGetFirstQueuedThread() {
	 ...
	 */
	Node t = tail; 
	if (t is h)                       // Empty queue
	  return null;

	if (t !is null) {
	  Thread tt = t.thread;
	  Node tp = t.prev;
	  if (tt !is null && tp is head)
	    return tt;
	}
      }
    }
  ...
  final bool isQueued(Thread thread) {
    ...
    for (Node p = v_tail; p !is null;) {
      volatile {
	if (p.thread is thread)
	  return true;
	p = p.prev;
      }
    }
  ...
  final bool transferForNotify(Node node) {
     ...
     */
    if (!Atomic.compareAndSet32(&node.waitStatus, Node.CONDITION, 0)) 
      return false;
     ...
     */
    Node p = enq(node);
    int c = p.v_waitStatus;
    if (c > 0 || !Atomic.compareAndSet32(&p.waitStatus, c, Node.SIGNAL)) {
      Thread t = node.v_thread;
      if (t) t.resume();
    }
    return true;
  }
  ...
  final int fullyRelease(Node node) {
	...
	return savedState;
      ...
      node.v_waitStatus = Node.CANCELLED;
      throw ex;
    }
   ...
   *
   * <p> Method documentation for this class describes mechanics, not
   ...
   */
  static class ConditionObject : Condition {
    /** First node of condition queue. */
    private Node firstWaiter;
    /** Last node of condition queue. */
    private Node lastWaiter;
    private AbstractLock parent;
    /**
    ...
    private Node addConditionWaiter() {
      Node node = new Node(Thread.getThis(), Node.CONDITION);
      Node t = lastWaiter;
      if (t is null) 
	firstWaiter = node;
      else 
     ...
     */
    private void doNotify(Node first) {
      do {
	if ( (firstWaiter = first.nextWaiter) is null) 
	  lastWaiter = null;
	first.nextWaiter = null;
      } while (!parent.transferForNotify(first) &&
     ...
     * \param first (non-null) the first node on condition queue
     */
    private void doNotifyAll(Node first) {
      lastWaiter = firstWaiter  = null;
      do {
	Node next = first.nextWaiter;
	first.nextWaiter = null;
	parent.transferForNotify(first);
	first = next;
      } while (first !is null);
    }
     ...
     * <ol>
     * <li> Save lock state returned by state()
...
version (Ares)
      ...
      parent.acquireQueued(node, savedState);
    }
     ...
     * <li> Save lock state returned by state()
     * <li> Invoke release with saved state as argument
     * <li> Block until signalled or timed out
     * <li> Reacquire by invoking specialized version of
     *      acquire with saved state as argument.
     * </ol>
     */
	...
	lastTime = now;
      ...
      parent.acquireQueued(node, savedState);
      return nanosTimeout - (currentTimeNanos() - lastTime);
    }
    ...
    final bool wait(long time, TimeUnit unit) {
      long nanosTimeout = toNanos(time,unit);
      Node node = addConditionWaiter();
      int savedState = parent.fullyRelease(node);
      long lastTime = currentTimeNanos();
      bool timedout = false;
    ...
    final bool isOwnedBy(AbstractLock sync) {
      // BVH: todo
      return sync is parent;
    }

    /**
     * Queries whether any threads are waiting on this condition.
     * Implements AbstractLock.hasWaiters
     * \return <tt>true</tt> if there are any waiting threads.
     */ 
     ...
     * this condition. 
     * Implements AbstractLock.getWaitQueueLength
     * \return the estimated number of waiting threads.
     */ 
     ...
     * Implements AbstractLock.getWaitingThreads
     * \return the collection of threads
     */
