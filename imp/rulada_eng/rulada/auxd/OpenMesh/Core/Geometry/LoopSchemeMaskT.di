...
/*****************************************************************************
 * Implements the weights needed for Loop subdivision. 
 *
...
module auxd.OpenMesh.Core.Geometry.LoopSchemeMaskT;
...
*/
class LoopSchemeMaskT(T_, uint cache_size_ = 100)
{
public:
...
protected:
    ...
    Scalar[cache_size]                       proj_weights_;
    Scalar[cache_size]                       limit_weights_;
    Scalar[cache_size]                       step_weights_;
    Scalar[][cache_size]                     tang0_weights_;
    Scalar[][cache_size]                     tang1_weights_;
...
protected:
    ...
    static Scalar                      compute_proj_weight(uint _valence)
    {
        ...
        double denom = (3.0 + 2.0*Math.cos(2.0*Math.PI/cast(double)_valence));
    ...
    {
        return Math.cos(2.0*Math.PI*cast(double)_ver_id/cast(double)_valence);
    }

    static Scalar                      compute_tang1_weight(uint _valence, uint _ver_id)
    {
        return Math.sin(2.0*Math.PI*cast(double)_ver_id/cast(double)_valence);
    }
    ...
    void                                      cache_weights()
    {
        ...
        for (uint k = 1; k < cache_size; ++k)
        {
            ...
            tang0_weights_[k].length = k;
            tang1_weights_[k].length = k;
            for (uint i = 0; i < k; ++i)
            {
...
public:
    ...
    {
        assert(_valence < cache_size);
        return Math.pow(step_weights_[_valence], cast(Scalar)_step);//can be precomputed
    }
...
alias LoopSchemeMaskT!(double, 100)        LoopSchemeMaskDouble;
alias SingletonT!(LoopSchemeMaskDouble)    LoopSchemeMaskDoubleSingleton;
