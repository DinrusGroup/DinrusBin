...
main()
{
    ...
    ver = GetVersion();
    dynamic_object* obj = cast(dynamic_object*) process_modules[IMAGE_SELF];
    ...
    call2[&D] = hook(cast(dynamic_object*)"kernel32.dll", "GetVersion", &D); assert(0x0D == GetVersion());
    call2[&E] = hook(cast(dynamic_object*)"kernel32.dll", "GetVersion", &E); assert(0x0E == GetVersion());
    unhook(cast(dynamic_object*)"kernel32.dll", "GetVersion");               assert(0x0D == GetVersion());
    unhook(cast(dynamic_object*)"kernel32.dll", "GetVersion");               assert(ver == GetVersion());
}

extern (Windows):

std.stdint.uint32_t GetVersion();
std.stdint.uint32_t function()[std.stdint.uint32_t function()] call2;
std.stdint.uint32_t D(){ assert(ver == call2[&D]()); return 0x0D; }
std.stdint.uint32_t E(){ assert(0x0D   == call2[&E]()); return 0x0E; }
}
