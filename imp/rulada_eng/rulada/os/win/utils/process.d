module os.win.utils.process;

import os.win.base.core,
  os.win.base.string,
  os.win.base.threading,
  os.win.base.native, //os.windows,
  os.win.com.core,
  os.win.sec.crypto;
static import os.win.io.path;
import std.c : malloc, realloc, free;

//alias HANDLE Handle;
extern(Windows){ int CloseHandle(Handle hObject);
...
class Process {
  ...
  final bool start() {
    ...
    auto pWorkingDirectory = os.win.io.path.currentDirectory().toUtf16z();
  ...
  private static ProcessInfo[] getProcessInfos() {
    ...
    int status;
    do {
      buffer = cast(ubyte*)std.c.realloc(buffer, bufferSize);
