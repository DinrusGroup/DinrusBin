...
private class CrcStream : CopyFilterStream {
  ...
  override size_t readBlock(void* buffer, size_t size) {
    ...
    if (n != 0)
      value = auxc.zlib.crc32(value, cast(ubyte*)buffer, n);
    return n;
  }
...
final class ZipReader {
  ...
  private void closeEntry() {
    ...
    if (entry_.data_ !is null) {
      input_.seek(cast(long)(entry_.compressedSize_ - entry_.data_.position), SeekPos.Current);
    }
    else {
  ...
  private void readEndOfCentralDirectory() {
    ...
    input_.position = cast(ulong)end;
