...
alias StringComparison СравнениеТекста;
enum StringComparison {
  ...
  ТекущаяКультура				 = CurrentCulture,
  ТекущаяКультураИгнорРег 		= CurrentCultureIgnoreCase,
  ИнвариантКультура				= InvariantCulture,
  ИнварианКультураИгнорРег		= InvariantCultureIgnoreCase,
  Порядковый 					= Ordinal,
  ПорядковыйИгнорРег 			= OrdinalIgnoreCase,
  
}

alias DateTimeKind ВидДатыВремени;
enum DateTimeKind {
...
alias AppDomainManagerInitializationOptions ОпцииИнициализацииМенеджераДоменаПрил;
enum AppDomainManagerInitializationOptions {

  None = 0x00000000,
  RegisterWithHost = 0x00000001,
  
  НУ							= None,
  РегистрироватьПоХосту 		= RegisterWithHost,
}

alias LoaderOptimization ОптимизацияЗагрузчика;
enum LoaderOptimization {

  NotSpecified = 0x00000000,
  SingleDomain = 0x00000001,
  MultiDomain = 0x00000002,
  MultiDomainHost = 0x00000003,
  DomainMask = 0x00000003,
  DisallowBindings = 0x00000004,
  
  НУ				= NotSpecified,
  ОдинДомен			= SingleDomain,
  МногоДомен		= MultiDomain,
  МногоДоменХост	= MultiDomainHost,
  МаскаДомена		= DomainMask,
  ЗапретПодвязок	= DisallowBindings,
}

alias AttributeTargets ЦелиАтрибутов;
enum AttributeTargets {
  ...
  Параметр		= Parameter,
  Делегат		= Delegate,
  ЗначениеВозврата	= ReturnValue,
  ГенерныйПараметр	= GenericParameter,
  Все				= All,
}

alias DayOfWeek ДеньНедели;
enum DayOfWeek {
...
alias EnvironmentVariableTarget ЦельПеременнойСреды;
...
alias ThreadPriority ПриоритетНити;
enum ThreadPriority {
...
alias ThreadState СостояниеНити;
enum ThreadState {
...
alias ApartmentState СостояниеКупе;
enum ApartmentState {
...
alias DebuggingModes РежимыОтладки;
enum DebuggingModes {
...
alias SymAddressKind ВидАдресаСим;
enum SymAddressKind {

  ILOffset = 0x00000001,
  NativeRVA = 0x00000002,
  NativeRegister = 0x00000003,
  NativeRegisterRelative = 0x00000004,
  NativeOffset = 0x00000005,
  NativeRegisterRegister = 0x00000006,
  NativeRegisterStack = 0x00000007,
  NativeStackRegister = 0x00000008,
  BitField = 0x00000009,
  NativeSectionOffset = 0x0000000A,
}
...
alias ProcessorArchitecture АрхитектураПроцессора;
...
enum PortableExecutableKinds {
  NotAPortableExecutableImage = 0x00000000,
  ILOnly = 0x00000001,
...
interface _MethodBase : IUnknown {
  ...
  /*[id(0x60010014)]*/ int Invoke_2(VARIANT obj, BindingFlags invokeAttr, _Binder Binder,   SAFEARRAY parameters, _CultureInfo culture, out VARIANT pRetVal);
  ...
  /*[id(0x60010022)]*/ int Invoke_3(VARIANT obj,   SAFEARRAY parameters, out VARIANT pRetVal);
}
...
interface _PropertyInfo : IUnknown {
  ...
  /*[id(0x60010010)]*/ int GetValue(VARIANT obj,   SAFEARRAY index, out VARIANT pRetVal);
  /*[id(0x60010011)]*/ int GetValue_2(VARIANT obj, BindingFlags invokeAttr, _Binder Binder,   SAFEARRAY index, _CultureInfo culture, out VARIANT pRetVal);
  /*[id(0x60010012)]*/ int SetValue(VARIANT obj, VARIANT value,   SAFEARRAY index);
  /*[id(0x60010013)]*/ int SetValue_2(VARIANT obj, VARIANT value, BindingFlags invokeAttr, _Binder Binder,   SAFEARRAY index, _CultureInfo culture);
