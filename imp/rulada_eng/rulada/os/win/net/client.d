...
extern (Windows) void CloseInternetHandle(Handle hwnd);
 ...
 * Параметры: address = The URI from which to download data.
 * Возвращает: A ubyte array containing the downloaded resource.
 * Примеры:
 * ---
 * import os.win.net.core, std.io;
 *
 * void main() {
 ...
 * Параметры: address = The URI from which to download.
 * Возвращает: A string containing the downloaded resource.
 */
 ...
 * Примеры:
 * ---
 * import os.win.net.core, os.win.io.path, std.io;
 *
 * void main() {
  ...
  if (!async) {
    if (schemeIsFtp) {
      ...
      if (statusCode > HTTP_STATUS_OK) {
        ...
        auto buffer = cast(wchar*)malloc(len * wchar.sizeof);
        scope(exit) free(buffer);

        HttpQueryInfo(request, HTTP_QUERY_STATUS_TEXT, buffer, &len, null);
        string statusText = toUtf8(buffer[0 .. len]);

        throw new NetException(std.string.format("Удалённый сервер вернул ошибку: (%s) %s", statusCode, statusText));
      }
 ...
 * Возвращает: A ubyte array containing the body of the response from the resource.
 */
 ...
 * Возвращает: A string containing the body of the response from the resource.
 */
