 ...
 */
module os.win.base.core;

alias Handle Описатель;
alias offsetof смещение_у;
alias Struct Структура;
alias IDisposable ИВымещаемый;
alias using используя;
alias cloneObject клонируйОбъект;
alias Optional Опционал;
alias ArgumentException ИсклАргумента;
alias ArgumentNullException ИсклНулевогоАргумента;
alias ArgumentOutOfRangeException ИсклАргументВнеОхвата;
alias FormatException ИсклФормата;
alias InvalidCastException ИсклПриведенияКТипу;
alias InvalidOperationException ИсклНеправильнОперации;
alias NotImplementedException ИсклНеРеализовано;
alias NotSupportedException ИсклНеПоддерживается;
alias NullReferenceException ИсклНулевойССылки;
alias UnauthorizedAccessException ИсклНесанкционированныйДоступ;
alias SecurityException ИсклБезопасности;
alias ArithmeticException АрифмИскл; 
alias OverflowException ИсклПереполнения;
...
alias opCall опВызова;
alias opAssign опПрисвоить;
alias value значение;
alias hasValue естьЗначение_ли;
alias opCmp опСравнить;
alias opEquals опРавны_ли;
