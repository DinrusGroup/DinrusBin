 ...
 */
module os.win.base.threading;

import os.windows: INFINITE;
...
class ThreadLocal(T) {
   ...
   * Параметры: value = The _value to be stored in the current thread's copy of this instance.
   */
 ...
 * Параметры: milliseconds = The number of _milliseconds for which the thread is blocked. Specify -1 to block the thread indefinitely.
 */
...
/**
 * Suspends the current thread for a specified time.
 * Параметры: timeout = The amount of time for which the thread is blocked. Specify -1 to block the thread indefinitely.
 */
...
abstract class WaitHandle {
  ...
  bool waitOne(uint millisecondsTimeout = INFINITE) {
    uint r = WaitForSingleObjectEx(handle_, millisecondsTimeout, cast(BOOL)1);
  ...
  static bool waitAll(WaitHandle[] waitHandles, uint millisecondsTimeout = INFINITE) {
    ...
    uint r = WaitForMultipleObjectsEx(handles.length, handles.ptr, cast(BOOL)1, millisecondsTimeout, cast(BOOL)1);
    return (r != WAIT_ABANDONED && r != WAIT_TIMEOUT);
  }
