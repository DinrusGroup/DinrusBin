...
version(Tango)
{
	...
	{
		pragma(msg, "os.win.gui: warning: this Tango version might not support DFL_BOTH_STRINGS");
	}
	else
	{
...
else
{
	...
	//alias os.win.charset.useWfuncs useUnicode; // D2 has this in std.file.
	//alias useWfuncs useUnicode; // D1 has it in both, causing a conflict.
	// os.win.charset is a better place for it, so use that one if present.
	static if(is(typeof(&os.win.charset.useWfuncs)))
		alias os.win.charset.useWfuncs useUnicode;
	else
...
Dstring getCommandLine()
{
	...
	return os.win.gui.x.utf.fromUnicodez(GetCommandLineW());
}
