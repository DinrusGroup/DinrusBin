...
module amigos.dmdscript.dglobal;

import std.uri;
import std.c;
import std.io;
import std.math;
import std.c: getchar;
...
version (none)
{
    ...
    // The this value то же самое, что the this value of the
    // calling context.
...
void* Dglobal_isFinite(Dobject pthis, CallContext *cc, Dobject othis, Value* ret, Value[] arglist)
{
	...
	v = &vundefined;
    n = v.toNumber();
    b = std.math.isfinite(n) ? true : false;
    ret.putVboolean(b);
    return null;
}
