...
module amigos.minid.commandline;
...
struct CLI(Input)
{
	...
	{
		// Check that amigos.minid.commandline.oldExits exists
		getRegistry(t);
		pushString(t, "amigos.minid.commandline.oldExits");
		...
		if(findGlobal(t, "exit"))
		{
			...
			dup(t);
			fielda(t, -4, "old");
			getRegistryVar(t, "amigos.minid.commandline.oldExits");
			swap(t);
			cateq(t, -2, 1);
			pop(t);
			swap(t);
	...
	private void cleanupExit(MDThread* t)
	{
		if(mReplacedExit)
		{
			// exit = registry.("amigos.minid.commandline.oldExits").pop()
			getRegistryVar(t, "amigos.minid.commandline.oldExits");
			pushNull(t);
