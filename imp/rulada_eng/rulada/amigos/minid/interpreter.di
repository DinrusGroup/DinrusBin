...
module amigos.minid.interpreter;
...
import amigos.minid.func;
import amigos.minid.gc;
import amigos.minid.instance;
import amigos.minid.namespace;
import amigos.minid.nativeobj;
import amigos.minid.opcodes;
import amigos.minid.string;
import amigos.minid.table;
import amigos.minid.thread;
import amigos.minid.types;
import amigos.minid.utils;
import amigos.minid.weakref;
...
call $(D removeRef) when your code no longer needs the object.  See $(amigos.minid.ex) for some reference helpers.
...
calls it), except that the destination buffer is a MiniD amigos.minid.string.

Params:
	fmt = The Tango-style format amigos.minid.string.
	... = The arguments to be formatted.

Returns:
	The stack index of the newly-pushed amigos.minid.string.
*/
...
/**
Sees if the value at the given _slot is a amigos.minid.string.
*/
...
enumeration defined in amigos.minid.types.
*/
...
/**
A shortcut for the very common case where you want to throw a formatted amigos.minid.string.  This is equivalent to calling
...
this function is called again, you will get an empty amigos.minid.string.

Returns:
	The stack index of the newly-pushed traceback amigos.minid.string.
*/
...
Returns:
	The stack index of the newly-pushed name amigos.minid.string.
*/
...
Params:
	slot = The stack index of the value to convert to a amigos.minid.string.
...
the stack, similar to idxa.  Throws an error if the field name is not a amigos.minid.string.  Pops both the set value and the
...
MDString* createString(MDThread* t, char[] data)
{
	...
	if(auto s = amigos.minid.string.string.lookup(t, data, h))
		return s;
	...
	return amigos.minid.string.string.create(t, data, h, cpLen);
}
	...
	switch(container.type)
	{
		case MDValue.Type.String:
			if(item.type == MDValue.Type.Char)
				return amigos.minid.string.string.contains(container.mString, item.mChar);
			else if(item.type == MDValue.Type.String)
				return amigos.minid.string.string.contains(container.mString, item.mString.toString());
			else
			{
				typeString(t, item);
...
void idxImpl(MDThread* t, MDValue* dest, MDValue* container, MDValue* key, bool raw)
{
	switch(container.type)
	{
		...
		case MDValue.Type.String:
			...
			*dest = amigos.minid.string.string.charAt(str, cast(uword)index);
			return;
	...
	if(a.type == b.type)
	{
		switch(a.type)
		{
			...
			case MDValue.Type.String:
				...
				return amigos.minid.string.string.compare(a.mString, b.mString);
...
void sliceImpl(MDThread* t, MDValue* dest, MDValue* src, MDValue* lo, MDValue* hi)
{
	switch(src.type)
	{
		...
		case MDValue.Type.String:
			...
			return *dest = amigos.minid.string.string.slice(t, str, cast(uword)loIndex, cast(uword)hiIndex);
