...
version (build) {pragma(link, "rulada");}
    /*version (DigitalMars) {
        ...
        version (Python_2_5_Or_Later) {
            pragma(link, "python2.5");//python2.5
        } else {
...
typeof(Ptr) lazy_sys(alias Ptr, string name) () {
    ...
    assert (Ptr !is null, "amigos.pyd.python.d couldn't load " ~ name ~ " attribute!");
    return Ptr;
}
...
typeof(Ptr) lazy_eval(alias Ptr, string code) () {
    ...
    assert (Ptr !is null, "amigos.pyd.python.d couldn't lazily eval something...");
    return Ptr;
}
...
c_str Py_FileSystemDefaultEncoding() {
    if (m_Py_FileSystemDefaultEncoding is null) {
        ...
        assert (m_Py_FileSystemDefaultEncoding !is null,
            "amigos.pyd.python.d couldn't load PyUnicode_DefaultEncoding attribute!");
    }
