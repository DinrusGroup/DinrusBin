...
private import rt.core.stdc.stdlib : alloca, rand;
...
version( TangoDoc )
{
     ...
     */
    int contains( Elem[] buf, Elem pat, Pred2E pred = Pred2E.init );
...
else
{
    ...
    {
        int contains( Buf buf, Pat pat )
        {
            return cast(int)(find( buf, pat ) != buf.length);
        }
    }
    ...
    {
        int contains( Buf buf, Pat pat, Pred pred )
        {
            return cast(int)(find( buf, pat, pred ) != buf.length);
        }
    }
