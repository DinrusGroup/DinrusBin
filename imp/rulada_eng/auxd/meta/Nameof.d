 ...
 *      a mixin hack, but it's so ugly that it cannot be recommendeauxd.
 *   2. The name mangling for symbols declared inside extern(Windows), extern(C) and extern(Pascal)
 *      functions is inherently ambiguous, so such inner symbols are not always correctly displayeauxd.
 *
 * License:   BSD style: $(LICENSE)
 * Authors:   Don Clugston
 * Copyright: Copyright (C) 2005-2006 Don Clugston
 */
module auxd.meta.Nameof;
private import auxd.meta.Demangle;
...
private {
    ...
    // This works because any class always needs to be fully qualifieauxd.
...
// This is just to ensure that the static assert actually gets executeauxd.
