 ...
 */
module os.win.loc.conv;

private import os.win.base.core,
  os.win.loc.consts,
  os.win.loc.core,
  os.win.loc.time,
  os.win.loc.num;
...
string toString(T)(T value, string format = null, IFormatProvider provider = null) {
  ...
  static if (is(T == struct)) {
    ...
    else static if (is(T == os.win.com.core.Decimal))
  ...
  //!! else
    //!! throw new os.win.base.core.InvalidCastException("Cannot convert from '" ~ T.stringof ~ "' to 'string'.");
}
...
/**
 * Преобразует строковое представление числа в его числовой эквивалент. The return value indicates whether the conversion succeeded or failed.
 */
