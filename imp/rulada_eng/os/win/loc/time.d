 ...
 */
module os.win.loc.time;

import os.win.base.core,
  os.win.base.time,
  os.win.base.native,
  os.win.utils.registry,
  os.win.loc.consts,
  os.win.loc.core;
import std.c : memcpy, wcslen;
...
abstract class Calendar {
   ...
   * Параметры: time = The DateTime to read.
   */
...
package class CalendarData {
  ...
  private static bool enumCalendarInfo(string localeName, uint calendar, uint calType, out string[] result) {
    ...
    static int enumCalendarsProc(wchar* lpCalendarInfoString) {
      temp ~= toUTF8(lpCalendarInfoString[0 .. std.c.wcslen(lpCalendarInfoString)]);
      return 1;
    }
...
struct DateTime {
   ...
   * Параметры: value = A number of whole a fractional milliseconds.
   */
   ...
   * Параметры: value = A number of whole a fractional seconds.
   */
   ...
   * Параметры: value = A number of whole a fractional minutes.
   */
   ...
   * Параметры: value = A number of whole a fractional hours.
   */
   ...
   * Параметры: value = A number of whole a fractional days.
   */
   ...
   * Параметры: value = A number of months.
   */
   ...
   * Параметры: value = A number of years.
   */
   ...
   * Параметры: other = The instance to _compare.
   */
   ...
   * Параметры: other = The instance to compare to this instance.
   */
   ...
   * Параметры: fileTime = A Windows file time expressed in ticks.
   */
  static DateTime fromFileTime(long fileTime) {
    long utcTicks = fileTime + (DaysTo1601 * TicksPerDay);

    FILETIME utcFileTime, localFileTime;
    utcFileTime.dwHighDateTime = (cast(uint)(utcTicks >> 32)) & 0xFFFFFFFF;
    utcFileTime.dwLowDateTime = (cast(uint)utcTicks) & 0xFFFFFFFF;
    ...
    switch (c) {
      ...
      case 'f', 'F':
        ...
        if (len <= 7) {
          ...
          result ~= os.win.loc.num.formatInt(cast(int)frac, fixedFormats[len - 1], Culture.constant);
        }
        else
          throw new FormatException("Input string was invalid.");
        break;
