...
/// Event handling.
module os.win.gui.event;

import os.win.gui.x.dlib;
...
template Event(T1, T2) // docmain
{
	...
	struct Event // docmain
	{
		...
		{
			static assert(is(TDG == delegate), "os.win.gui: Event handler must be a delegate");
			
			alias ParameterTypeTuple!(TDG) TDGParams;
			static assert(TDGParams.length == 2, "os.win.gui: Event handler needs exactly 2 parameters");
			
			static if(is(TDGParams[0] : Object))
			{
				static assert(is(T1: TDGParams[0]), "os.win.gui: Event handler parameter 1 type mismatch");
			}
			else
			{
				static assert(is(T1 == TDGParams[0]), "os.win.gui: Event handler parameter 1 type mismatch");
			...
			{
				static assert(is(T2 : TDGParams[1]), "os.win.gui: Event handler parameter 2 type mismatch");
			}
			else
			{
				static assert(is(T2 == TDGParams[1]), "os.win.gui: Event handler parameter 2 type mismatch");
