...
private import std.io, std.string, std.path, std.file,
	std.random, std.cstream, std.stream;
private import std.process;
private import std.c;

private import os.win.gui.all, os.win.gui.x.winapi, os.win.gui.x.utf;
...
bool optBuild = false; // Build os.win.gui.lib.
...
{
	char[] libdir;
	libdir = std.path.join(dir, "lib");
	if(std.path.isabs(dir) && std.file.exists(std.path.join(dir, "bin\\dmd.exe")) &&
		std.file.exists(libdir) && isdir(libdir))
		return true;
	return false;
}
...
bool doDflSwitch(char[] arg)
{
	...
	switch(arg)
	{
		case "dmd":
			prepare();
			std.process.system(quotearg(std.path.join(dmdpath, "bin\\dmd.exe\"")));
			exit(0);
...
char[] getshortpath(char[] fn)
{
	if(os.win.gui.x.utf.useUnicode)
	{
		...
		len = proc(os.win.gui.x.utf.toUnicodez(fn), s.ptr, s.length);
	...
	else
	{
		...
		char[] s;
		s = new char[MAX_PATH];
		len = GetShortPathNameA(os.win.gui.x.utf.toAnsiz(fn), s.ptr, s.length);
...
int main(/+ char[][] args +/)
{
	...
	if(args.length > 1)
	{
		...
		void findimportdir()
		{
			if(!importdir.length)
			{
				...
				if(!std.file.exists(importdir) || !std.file.exists(std.path.join(importdir, "dfl")))
				{
					...
					if(!std.file.exists(importdir) || !std.file.exists(std.path.join(importdir, "dfl")))
					{
						importdir = std.path.join(dmdpath, "srcs\\dfl");
						if(!std.file.exists(importdir) || !std.file.exists(std.path.join(importdir, "dfl")))
						{
							importdir = std.path.join(dmdpath, "bin\\import");
							if(!std.file.exists(importdir) || !std.file.exists(std.path.join(importdir, "dfl")))
							{
								importdir = null;
		...
		void buildDflLibs()
		{
			...
			char[] dmcpath;
			if(std.file.exists(std.path.join(dmdpath, "bin\\link.exe"))
				&& std.file.exists(std.path.join(dmdpath, "bin\\lib.exe")))
			{
				dmcpath = dmdpath;
			}
			else
			...
			{
				batf.writeString("\r\n   @set _old_path=%path%");
				batf.writeString("\r\n   @set path=%dmd_path%;%dmc_path%;%path%");
				batf.writeString("\r\n   @set dflc=true");
			...
			batf.writeString("\r\n   @move /Y dfl*.lib %dmd_path%\\lib > NUL"); // Important! no longer using go.bat for this.
		...
		void findlibdir()
		{
			...
			if(!dfllib.length)
			{
				...
				{
					dfllib = std.path.join(dmdpath, "lib\\" ~ libfile);
					if(!std.file.exists(dfllib))
					{
						dfllib = null;
		...
		void doVerCheck(bool vcVerbose = false, bool vcPrintIssues = true)
		{
			...
			std.process.system(getshortpath(std.path.join(dmdpath, "bin\\dmd.exe"))
				~ " > " ~ x2);
		...
		else
		{
			...
			// Call DMD.
			assert(dmdpath.length);
			char[] cmdline;
			int sc;
			cmdline = getshortpath(std.path.join(dmdpath, "bin\\dmd.exe")) ~ " " ~ std.string.join(dmdargs, " ");
			writefln("%s", cmdline);
