      ...
      with (codecs[i] = new ImageCodecInfo) {
        with (pCodecs[i]) {
          ...
          for (int j = 0; j < SigCount; j++) {
            ...
            std.c.memcpy(signaturePatterns[j].ptr, SigPattern + (j * SigSize), SigSize);
            std.c.memcpy(signatureMasks[j].ptr, SigMask + (j * SigSize), SigSize);
          }
        }
      }
    }
