/******************************************************************************* 
	...
	BasicArchive template allows for reading and writing of different 
		data types.
	...
	Description:    
		BasicArchive template allows for reading and writing of different 
	data types. Not to be used by itself.
...
module arc.serialization.basicarchive;

import arc.serialization.classregister;
...
protected:
	...
	void describe_class(T)(inout T x)
	{
		if (write_or_read == WriteRead.Write)
		{
			...
			{
				// if T is default constructible, not registering isn't an error
		...
		else if (write_or_read == WriteRead.Read)
		{
			...
			{
				//FIXME: If throwing Exception, archive should be in ok state!
				...
				if(name == T.mangleof)
				{
					static if( is(typeof(new T)) )
						...
						throw new Exception("Trying to read unregistered class - probably " ~ T.classinfo.name ~ 
															 ...
															 " is not default-constructible, so you need to register it ahead of time.");
				}
				else
				{
