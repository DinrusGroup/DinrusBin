 ...
 *
 * Набор допускатьДубликаты to true to allow duplicate values to be inserted.
 */
struct КЧДерево(V, alias функСравнить, alias обновлФункц, alias Разместитель=ДефолтныйРазместитель, бул допускатьДубликаты=false, бул обновлять_ли=true)
{
    ...
    Узел найди(V v)
    {
        ...
        else
        {
            Узел n = конец.лево;
            цел резсравн;
            while(n !is null && (резсравн = функСравнить(n.значение, v)) != 0)
            {
                if(резсравн < 0)
                    n = n.право;
                else
    ...
    static if(допускатьДубликаты)
    {
        ...
        /**
         * удали all the nodes that match v
         *
         * Runs in O(m * lg(n)) where m is the number of v экземпляры in the
         * collection, and n is the счёт of the collection.
         */
