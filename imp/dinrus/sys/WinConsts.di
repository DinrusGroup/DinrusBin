...
ПТипЗКД, ПМайлСлот, ППраваДоступа, ПТипТокена, ПТипБид,
 ПУровеньИмперсонацииБезопасности, ПТокен, КЛАСС_ИНФОРМАЦИИ_ТОКЕНА,
  ПСтд, ПШирСим, ПСжатие, ПРежимКонсоли, ППозКурсора, ППозПотока,
  ПРеестр, ПКлючРег, ПФайл, ПСовмИспФайла, ПРежимФайла, ПРежСоздФайла,
   ПФлагКУДоп, ППриоритетНити, ППроцесс, ПРежим_Адресации, ПИдЧП, ПКомРез,
    ПОшибка, ПЗагрФлаг, ПОкПерерис, ПФичаБезопмаса, ПТипВарианта,
     ПВинСокОш, ПТипСок, ППротокол, ПОпцияСокета, ПСемействоАдресов,
      ПЭкстрЗакрытиеСокета, ПФлагиСокета, ПИнАдр, ПИницКо, ПИмИнфо,
      АИ, ПСисМетрика, ПАтрыИнфосим, ПТипСоб, ПТипСобМыши, ПСостКнопкиМыши,
       ПСостКлУпр, ПСтильПера, ПЗвук, ППамять, ПФлагСооб, ПВиртКл, ППрокр,
        ПВидОкна, ПЦветУпрЭлта, ПСтильКласса, ПСлотПочты, ПСооб, ПФорматСооб,
         ПЯзык, ППодъяз, ПКодСтр, ПКонтекст, ПОкСооб, ПФлагСоздПроц,
          ПКонтекстВып, ППолитикаИсключений, ПДиспачФлаг, ПСредство, ПВар,
           ПАспектЦУ, ТИМЕД, ПВидТипа, ПКонвВызова, ПВидФунк, ПВидВызова,
            ПВидПерем, ПФлагТипа, ПФлагФунк, ПФлагиПерем, ПВидДескр,
             ПВидСистемы, ПВидРег, ПВидИзм, ПОшибкаАИ, ПФлагиАИ, ПФлагиУИ
*/
...
enum ПТипСок: цел
{
	...
	ДГрамма =      2, //DGRAM
	Необр =        3, //RAW
	НДС =        4, //RDM: Надёжно Доставленное Сообщение (RDM  - Reliably Delivered Message)
	ППП =  5, //SEQPACKET: Последовательный Поток Пакетов
}
...
{
	Отладка =                0x1,///SO_DEBUG: записать отладочную инфо
...
{
	Неук =       0,             /// NONE :флагов не указано
	СПД =        0x1,       /// OOB: сверхпакетные данные потока
	Просмотр =       0x2,      ///PEEK: посмотреть входящие данные, не удаляя их из очереди, только при получении
	НеМаршрутизировать =  0x4, ///DONTROUTE: данные не следует подвергать маршрутизации; этот флаг может игнорироваться. Только для отправки.
...
enum ПФлагиАИ: цел
{
	     Пассив = 0x00000001,               /// Сокет адрес will be used in свяжи() вызов
         КанонИмя = 0x00000002,             /// Return canonical имя in first ai_canonname
         НумерикХост = 0x00000004 ,          /// Nodename must be a numeric адрес ткст
         НумерикСлужба = 0x00000008,           /// Servicename must be a numeric порт число
         Все = 0x00000100,                   /// Query Всё ИП6 and ИП4 with AI_V4MAPPED
         КонфигАдра = 0x00000400,            /// Resolution only if global адрес configured
         В4Картир = 0x00000800,              /// On v6 failure, query v4 and преобразуй на_это V4MAPPED форматируй
...
enum ПФлагиУИ: цел
{
        МаксХост = 1025,
        МаксСерв = 32,
        НумерикХост = 0x01,                  /// Don't try на_это look up имя_хоста.
        НумерикСлужба = 0x02,                  /// Don't преобразуй порт число на_это имя.
        НетПКДН = 0x04,                       /// Only return nodename portion.
        ЗапрВозврИм = 0x08,                     /// Don't return numeric адресes.
        ДГрамма = 0x10,                        /// Look up ППД служба rather than ПУТ.
}

enum ПОшибкаАИ
{
         ПлохиеФлаги = 10022,                   /// EAI_BADFLAGS: неверное значение поля `ai_flags'.
         НетИмени = 11001,                     /// EAI_NONAME: NAME or SERVICE is неизвестное.
         ПриРазрешенииАдр = 11002,                      ///EAI_AGAIN: Temporary failure in имя resolution.
         Неудачно = 11003,                       ///EAI_FAIL: Non-recoverable failure in имя рез.
         НетДанных = 11001,                     ///EAI_NODATA: No адрес associated with NAME.
         Семейство = 10047,                     ///EAI_FAMILY: `ai_family' not supported.
         ТипСокета = 10044,                   /// EAI_SOCKTYPE: `ai_socktype' not supported.
         Служба = 10109,                    ///EAI_SERVICE: SERVICE not supported for `ai_socktype'.
         Память = 8,                         ///EAI_MEMORY: Memory allocation failure.
}
