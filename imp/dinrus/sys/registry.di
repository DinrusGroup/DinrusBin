...
/+ ++++++ This is borrowed from synsoft.text.token, until such time as something
 ...
 +/
ткст[] tokenise(ткст source, сим delimiter, булев bElideBlanks, булев bZeroTerminate)
{
    цел         i;
    цел         cDelimiters =   128;
...
public enum REG_VALUE_TYPE
{
    ...
    ,   REG_SZ                          =   1  //!< A zero-terminated ткст
    ,   REG_EXPAND_SZ                   =   2  //!< A zero-terminated ткст containing expandable environment variable references
...
/+
private ткст expand_environment_strings(in ткст value)
in
{
...
body
{
    return RegCreateKeyExA( hkey, вТкст0(subKey), RESERVED, RESERVED
...
body
{
    return RegDeleteKeyA(hkey, вТкст0(subKey));
}
...
body
{
    return RegDeleteValueA(hkey, вТкст0(valueName));
}
...
private LONG Reg_EnumKeyName_(  in HKEY hkey, in DWORD index, inout сим [] name
                            ,   out DWORD cchName)
in
{
    assert(!(null is hkey));
...
private LONG Reg_GetValueType_( in HKEY hkey, in ткст name
...
{
    DWORD   cbData  =   0;
    LONG    res     =   RegQueryValueExA(   hkey, вТкст0(name), RESERVED, type
                                        ,   cast(byte*)0, cbData);
...
body
{
    return RegOpenKeyExA(hkey, вТкст0(subKey), RESERVED, samDesired, hkeyResult);
}

private void Reg_QueryValue_(   in HKEY hkey, ткст name, out ткст value
                            ,   out REG_VALUE_TYPE type)
in
{
    assert(!(null is hkey));
...
body
{
    ...
    LONG    res     =   RegQueryValueExA(   hkey, вТкст0(name), RESERVED
                                        ,   type, data, cbData);
    ...
    if(ERROR_MORE_DATA == res)
    {
        ...
        res = RegQueryValueExA( hkey, вТкст0(name), RESERVED, type, data
                            ,   cbData);
    }
    ...
    else
    {
        switch(type)
        {
            ...
            case    REG_VALUE_TYPE.REG_MULTI_SZ:
                throw new RegistryException("Cannot read the given value as a ткст");

            case    REG_VALUE_TYPE.REG_SZ:
            case    REG_VALUE_TYPE.REG_EXPAND_SZ:
                value = вТкст(cast(сим*)data);
		if (value.ptr == cast(сим*)&u.qw)
...
private void Reg_QueryValue_(   in HKEY hkey, in ткст name, out ткст[] value
                            ,   out REG_VALUE_TYPE type)
in
{
    assert(!(null is hkey));
...
body
{
    ...
    if(ERROR_MORE_DATA == res)
    {
        ...
        res = RegQueryValueExA(hkey, вТкст0(name), RESERVED, type, data.ptr, cbData);
    }
    ...
    value = tokenise(cast(ткст)data, cast(сим)0, 1, 0);
}

private void Reg_QueryValue_(   in HKEY hkey, in ткст name, out uint value
                            ,   out REG_VALUE_TYPE type)
in
{
    assert(!(null is hkey));
...
private void Reg_QueryValue_(   in HKEY hkey, in ткст name, out ulong value
                            ,   out REG_VALUE_TYPE type)
in
{
    assert(!(null is hkey));
...
private void Reg_QueryValue_(   in HKEY hkey, in ткст name, out byte[] value
                            ,   out REG_VALUE_TYPE type)
in
{
    assert(!(null is hkey));
...
private void Reg_SetValueExA_(  in HKEY hkey, in ткст subKey
                            ,   in REG_VALUE_TYPE type, in LPCVOID lpData
                            ,   in DWORD cbData)
in
{
...
body
{
    LONG    res =   RegSetValueExA( hkey, вТкст0(subKey), RESERVED, type
                                ,   lpData, cbData);
...
//@{
private:
    this(HKEY hkey, ткст name, булев created)
    in
    {
        assert(!(null is hkey));
    }
...
public:
    ...
    Key createKey(ткст name, REGSAM access)
    {
        if( null is name ||
            0 == name.length)
        {
    ...
    /// Returns the named sub-key of this key
    ///
    /// \param name The name of the subkey to aquire. If name is null (or the empty-ткст), then the called key is duplicated
    ...
    Key getKey(ткст name, REGSAM access)
    {
        if( null is name ||
            0 == name.length)
        {
    ...
    ///
    /// \note if name is null (or the empty-ткст), then the default value is returned
    ...
    ///
    /// \param name The name of the value to set. If null, or the empty ткст, sets the default value
    ...
    void setValue(ткст name, uint value)
    {
    ...
    /// \note If a value corresponding to the requested name is not found, a RegistryException is thrown
    void setValue(ткст name, uint value, Endian endian)
    {
    ...
    /// \note If a value corresponding to the requested name is not found, a RegistryException is thrown
    void setValue(ткст name, ulong value)
    {
        Reg_SetValueExA_(m_hkey, name, REG_VALUE_TYPE.REG_QWORD, &value, value.sizeof);
    }

    /// Sets the named value with the given ткст value
    ///
    /// \param name The name of the value to set. If null, or the empty ткст, sets the default value
    /// \param value The ткст value to set
    /// \note If a value corresponding to the requested name is not found, a RegistryException is thrown
    void setValue(ткст name, ткст value)
    {
    ...
    /// \param name The name of the value to set. If null, or the empty ткст, sets the default value
    /// \param value The ткст value to set
    /// \param asEXPAND_SZ If true, the value will be stored as an expandable environment ткст, otherwise as a normal ткст
    /// \note If a value corresponding to the requested name is not found, a RegistryException is thrown
    void setValue(ткст name, ткст value, булев asEXPAND_SZ)
    {
    ...
    /// \note If a value corresponding to the requested name is not found, a RegistryException is thrown
    void setValue(ткст name, byte[] value)
    {
...
private:
    this(Key key, ткст name, REG_VALUE_TYPE type)
    in
    {
        assert(!(key is null));
    }
...
public:
    ...
    ///
    /// \note If the value represents a default value of a key, which has no name, the returned ткст will be of zero length
    ткст name()
    {
        return m_name;
    }
    ...
    /// Obtains the current value of the value as a ткст.
    ///
...
/// <code></code>
/// <br>
/// <code>&nbsp;&nbsp;foreach(сим[] kName; key.SubKeys)</code>
/// <br>
/// <code>&nbsp;&nbsp;{</code>
/// <br>
...
public:
    цел opApply(цел delegate(inout ткст name) dg)
    {
        цел     result  =   0;
        HKEY    hkey    =   m_key.m_hkey;
        DWORD   cSubKeys;
        ...
        for(DWORD index = 0; 0 == result; ++index)
        {
            ...
            {
                ткст name = cast(ткст)sName[0 .. cchName];
...
public:
    ...
    Key getKey(uint index)
    {
        ...
        return m_key.getKey(cast(ткст)sName[0 .. cchName]);
    }
...
public:
    цел opApply(цел delegate(inout Key key) dg)
    {
        цел         result  =   0;
        HKEY        hkey    =   m_key.m_hkey;
        DWORD       cSubKeys;
	    ...
	    res     =   Reg_EnumKeyName_(hkey, index, sName, cchName);
            ...
            else if(ERROR_SUCCESS == res)
            {
                try
                {
                    Key key =   m_key.getKey(cast(ткст)sName[0 .. cchName]);
...
/// <code></code>
/// <br>
/// <code>&nbsp;&nbsp;foreach(сим[] vName; key.Values)</code>
/// <br>
/// <code>&nbsp;&nbsp;{</code>
/// <br>
...
public:
    ...
    Value getValue(uint index)
    {
        ...
        return m_key.getValue(cast(ткст)sName[0 .. cchName]);
    }
...
public:
    цел opApply(цел delegate(inout Value value) dg)
    {
        цел     result  =   0;
        HKEY    hkey    =   m_key.m_hkey;
        DWORD   cValues;
	    ...
	    res = Reg_EnumValueName_(hkey, index, sName.ptr, cchName);
            ...
            {
                Value value = m_key.getValue(cast(ткст)sName[0 .. cchName]);
