...
бул пустой(T)(ref T[] x)
{
    return x.length == 0;
}

проц пуш_бэк(T)(ref T[] x, ref T el)
{
    x ~= el;
}

проц поп_бэк(T)(ref T[] x)
{
    x.length = x.length-1;
}
...
struct ОбходчикМассива(T)
{
    ...
    static ОбходчикМассива начало(T[] x)
    {
        return ОбходчикМассива(x.укз);
    }
    static ОбходчикМассива конец(T[] x)
    {
        return ОбходчикМассива(x.укз + x.length);
    }
