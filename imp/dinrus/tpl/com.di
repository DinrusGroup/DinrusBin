...
/**
 * Определяет при компиляции встроенный тип, эквивалентный ОМО-типу.
 * Примеры:
 * ---
...
template com_cast_impl(T, ППолитикаИсключений политика)
{
    ...
    T com_cast_impl(U)(U объ)
    {
        static if (is(U : Инкогнито))
        {
            ...
            static if (политика == ППолитикаИсключений.Выводить)
                ошибка("Неудачный каст из '" ~ U.stringof ~ "' в '" ~ T.stringof ~ "'.");
            else
                return пусто;
        }
        ...
        else static if (is(U == ВАРИАНТ))
        {
            ...
            else static assert(нет, "Не удаётся каст из '" ~ U.stringof ~ "' в '" ~ T.stringof ~ "'.");
        }
        else static assert(нет, "Не удаётся каст из '" ~ U.stringof ~ "' в '" ~ T.stringof ~ "'.");
    }
...
/**
 * Вызывет операцию преобразования из одного типа ОМО в другой.
 *
...
/// Вызывает операцию преобразования из одного ОМО типа в другой, как и выше, но выводит
///исключение в случае неудачного каста.
/// Выводит исключение: ИсклКОМ, если каст не удался.

template com_safe_cast(T)
{
    alias com_cast_impl!(T, ППолитикаИсключений.Выводить) com_safe_cast;
}

/**
 * Создаёт ОМО объект класса, ассоциированного с указанным CLSIDом.
 * Параметры:
...
template создайКо(T, ППолитикаИсключений политика = cast(ППолитикаИсключений)1)
{
    ...
    T создайКо(U)(U клсид, ПКонтекстВып контекст = cast(ПКонтекстВып) 0x1)
    {
        ...
        T ret;
        цел хрез = СоздайЭкземплярКо(гуид, пусто, контекст, ууид_у!(T), возврзнач(ret));
...
template создайКоДоп(T, ППолитикаИсключений политика = cast(ППолитикаИсключений) 1)
{
    ...
    T создайКоДоп(U)(U клсид, ткст сервер, ПКонтекстВып контекст = cast(ПКонтекстВып) 0x1)
    {
        ...
        цел хрез = СоздайЭкземплярКоДоп(гуид, пусто, контекст, &кси, 1, &ret);
...
template СчётСсылокРеализ()
{
    ...
    extern(Windows):
        ...
        бцел AddRef()
    {
        return БлокированныйИнкремент(&refCount_);
    }
...
/**
 * Абстрактный класс-основа для объектов ОМО, происходящих от Инкогнито или ИДиспетчер.
 *
...
///////////////////////////////////////////////////////////////////////////////////////////////////
// DMD предотвращает запуск деструкторов над объектами ОМО.
            ...
            if (*ci)
            {
                if (auto c = **ci)
                {
                    do
                    {
                        if (c.destructor)
                        {
                            auto финализатор = cast(проц function(Объект))c.destructor;
                            финализатор(объ);
                        }
                        c = c.base;
                    }
