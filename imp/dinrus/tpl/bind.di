...
struct Кортеж() {
	...
	alias ПустойКортеж_!()	тип;		
	alias ПустойКортеж_!()	значение;		
...
/*
	...
	Парамы:
		...
		Y = тип источника
		b = источник
		копироватьСтатМас = когда статический массив присваивается динамическому, иногда требуется его дублирование, так как хранилище может быть в "волатильных" местах...
*/
template _присвой(T, Y, бул копироватьСтатМас =да) {
	static if (статМас_ли!(T)) {
...
/**
	Dynamically create a кортеж from the given items
*/
Кортеж!(T) кортеж(T ...)(T t) {
	Кортеж!(T) рез;
	foreach (i, x; t) {
...
template типыДинАргов(цел i, ПарамыФункц, СвязанныеАрги, цел minParamsLeft) {
	...
	template prependType(T, SkipType) {
		...
		static if (is(T == void)) {	// проц means that we aren't adding anything
			alias tmp рез;
		} else {
