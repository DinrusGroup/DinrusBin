...
alias SERVICE_STATUS СТАТУС_СЛУЖБЫ;
struct SERVICE_STATUS {

alias dwServiceType типСлужбы;
alias dwCurrentState текущСостояние;
alias dwControlsAccepted принятыеКонтролы;
alias dwWin32ExitCode кодВыходаВин32;
alias dwServiceSpecificExitCode особыйКодВыходаСлужбы;
alias dwCheckPoint контрольнаяТочка;
alias dwWaitHint ожиданиеПодсказки;
...
бул ИзмениКонфигСлужбыА(ук служба, бцел типСлужбы, бцел типСтарта, бцел контрольОшибок,
	усим путьКБин, усим группаПорядкаЗагрузки, убцел идТега, усим зависимости, усим стартИмяСлужбы, усим пароль, усим отображаемоеИмя)
	{
	return cast(бул) ChangeServiceConfigA(cast(SC_HANDLE) служба, cast(DWORD) типСлужбы, cast(DWORD) типСтарта, cast(DWORD) контрольОшибок, cast(LPCSTR) путьКБин, cast(LPCSTR) группаПорядкаЗагрузки, cast(LPDWORD) идТега, cast(LPCSTR) зависимости, cast(LPCSTR) стартИмяСлужбы, cast(LPCSTR) пароль, cast(LPCSTR) отображаемоеИмя);
	 }

бул ИзмениКонфигСлужбы(ук служба, бцел типСлужбы, бцел типСтарта, бцел контрольОшибок,
	ушим путьКБин, ушим группаПорядкаЗагрузки, убцел идТега, ушим зависимости, ушим стартИмяСлужбы, ушим пароль, ушим отображаемоеИмя)
	{
	return cast(бул) ChangeServiceConfigW(cast(SC_HANDLE) служба, cast(DWORD) типСлужбы, cast(DWORD) типСтарта, cast(DWORD) контрольОшибок, cast(LPCWSTR) путьКБин, cast(LPCWSTR) группаПорядкаЗагрузки, cast(LPDWORD) идТега, cast(LPCWSTR) зависимости, cast(LPCWSTR) стартИмяСлужбы, cast(LPCWSTR) пароль, cast(LPCWSTR) отображаемоеИмя);
	 }
	  
бул ЗакройДескрипторСлужбы(ук служба)
	{
	return cast(бул) CloseServiceHandle(cast(SC_HANDLE) служба);
	}

бул ОтправьКодУпрСлужбе(ук служба, бцел кодУпр, СТАТУС_СЛУЖБЫ* сс)
	{
	return cast(бул) ControlService(cast(SC_HANDLE) служба, cast(DWORD) кодУпр, cast(LPSERVICE_STATUS) сс);
	}
	
ук СоздайСлужбуА(ук менеджерБДУпрСлужб, усим имяСлужбы, усим отобрИмя, бцел видДоступа, бцел типСлужбы, бцел стартТип, бцел контрОш, усим путьКБин, усим группаПорядкаЗагр, убцел идТега, усим зависимости, усим стартИмяСлужбы, усим пароль)
	{
	return cast(ук) CreateServiceA(cast(SC_HANDLE)менеджерБДУпрСлужб, cast(LPCSTR) имяСлужбы, cast(LPCSTR) отобрИмя, cast(DWORD) видДоступа, cast(DWORD) типСлужбы, cast(DWORD) стартТип, cast(DWORD) контрОш, cast(LPCSTR) путьКБин, cast(LPCSTR) группаПорядкаЗагр, cast(PDWORD) идТега, cast(LPCSTR) зависимости, cast(LPCSTR) стартИмяСлужбы, cast(LPCSTR) пароль);
	}
	 
ук СоздайСлужбу(ук менеджерБДУпрСлужб, ушим имяСлужбы, ушим отобрИмя, бцел видДоступа, бцел типСлужбы, бцел стартТип, бцел контрОш, ушим путьКБин, ушим группаПорядкаЗагр, убцел идТега, ушим зависимости, ушим стартИмяСлужбы, ушим пароль)
	{
	return cast(ук) CreateServiceW(cast(SC_HANDLE)менеджерБДУпрСлужб, cast(LPCWSTR) имяСлужбы, cast(LPCWSTR) отобрИмя, cast(DWORD) видДоступа, cast(DWORD) типСлужбы, cast(DWORD) стартТип, cast(DWORD) контрОш, cast(LPCWSTR) путьКБин, cast(LPCWSTR) группаПорядкаЗагр, cast(PDWORD) идТега, cast(LPCWSTR) зависимости, cast(LPCWSTR) стартИмяСлужбы, cast(LPCWSTR) пароль);
	}

бул УдалиСлужбу(ук служба)
	{
	return cast(бул) DeleteService(cast(SC_HANDLE) служба);
	}
/*
бул ПеречислиЗависимыеСлужбыА(ук, бцел , ПЕРЕЧЕНЬ_СТАТУСА_СЛУЖБ_А* ,бцел , убцел, убцел)
	{
	return cast(бул) EnumDependentServicesA(cast(SC_HANDLE), cast(DWORD), LPENUM_SERVICE_STATUSA, cast(DWORD), PDWORD, PDWORD);
	}

	BOOL EnumDependentServicesW(cast(SC_HANDLE), cast(DWORD), LPENUM_SERVICE_STATUSW,
	  cast(DWORD), PDWORD, PDWORD);
	BOOL EnumServicesStatusA(cast(SC_HANDLE), cast(DWORD), cast(DWORD), LPENUM_SERVICE_STATUSA,
	  cast(DWORD), PDWORD, PDWORD, PDWORD);
	BOOL EnumServicesStatusW(cast(SC_HANDLE), cast(DWORD), cast(DWORD), LPENUM_SERVICE_STATUSW,
	  cast(DWORD), PDWORD, PDWORD, PDWORD);
	BOOL GetServiceDisplayNameA(cast(SC_HANDLE), cast(LPCSTR), LPSTR, PDWORD);
	BOOL GetServiceDisplayNameW(cast(SC_HANDLE), LPCWSTR, LPWSTR, PDWORD);
	BOOL GetServiceKeyNameA(cast(SC_HANDLE), cast(LPCSTR), LPSTR, PDWORD);
	BOOL GetServiceKeyNameW(cast(SC_HANDLE), LPCWSTR, LPWSTR, PDWORD);
	SC_LOCK LockServiceDatabase(cast(SC_HANDLE));
	BOOL NotifyBootConfigStatus(BOOL);
	cast(SC_HANDLE) OpenSCManagerA(cast(LPCSTR), cast(LPCSTR), cast(DWORD));
	cast(SC_HANDLE) OpenSCManagerW(LPCWSTR, LPCWSTR, cast(DWORD));
	cast(SC_HANDLE) OpenServiceA(cast(SC_HANDLE), cast(LPCSTR), cast(DWORD));
	cast(SC_HANDLE) OpenServiceW(cast(SC_HANDLE), LPCWSTR, cast(DWORD));
	BOOL QueryServiceConfigA(cast(SC_HANDLE), LPQUERY_SERVICE_CONFIGA, cast(DWORD),
	  PDWORD);
	BOOL QueryServiceConfigW(cast(SC_HANDLE), LPQUERY_SERVICE_CONFIGW, cast(DWORD),
	  PDWORD);
	BOOL QueryServiceLockStatusA(cast(SC_HANDLE), LPQUERY_SERVICE_LOCK_STATUSA,
	  cast(DWORD), PDWORD);
	BOOL QueryServiceLockStatusW(cast(SC_HANDLE), LPQUERY_SERVICE_LOCK_STATUSW,
	  cast(DWORD), PDWORD);
	BOOL QueryServiceObjectSecurity(cast(SC_HANDLE), SECURITY_INFORMATION,
	  PSECURITY_DESCRIPTOR, cast(DWORD), cast(LPDWORD));
	BOOL QueryServiceStatus(cast(SC_HANDLE), LPSERVICE_STATUS);
	SERVICE_STATUS_HANDLE RegisterServiceCtrlHandlerA(cast(LPCSTR),
	  LPHANDLER_FUNCTION);
	SERVICE_STATUS_HANDLE RegisterServiceCtrlHandlerW(LPCWSTR,
	  LPHANDLER_FUNCTION);
	BOOL SetServiceObjectSecurity(cast(SC_HANDLE), SECURITY_INFORMATION,
	  PSECURITY_DESCRIPTOR);
	BOOL SetServiceStatus(SERVICE_STATUS_HANDLE, LPSERVICE_STATUS);
	BOOL StartServiceA(cast(SC_HANDLE), cast(DWORD), cast(LPCSTR)*);
	BOOL StartServiceW(cast(SC_HANDLE), cast(DWORD), LPCWSTR*);
	BOOL StartServiceCtrlDispatcherA(LPSERVICE_TABLE_ENTRYA);
	BOOL StartServiceCtrlDispatcherW(LPSERVICE_TABLE_ENTRYW);
	BOOL UnlockServiceDatabase(SC_LOCK);
	*/
