/*******************************************************************************
        ...
        замени (источник, свер, замена)        // замени chars
        ...
        locate (источник, свер, старт)               // найди сим
        locatePrior (источник, свер, старт)          // найди приор сим
        locatePattern (источник, свер, старт);       // найди образец
        locatePatternPrior (источник, свер, старт);  // найди приор образец
        индексУ (s*, свер, length)                 // low-уровень отыщи
        не_совпадают (s1*, s2*, length)                 // low-уровень сравни
        matching (s1*, s2*, length)                 // low-уровень сравни
        ...
        quotes (стр, установи)                           // foreach quotes
...
{
        return locatePattern (источник, свер) != источник.length;
}
...
{
        return (свер.length is 1) ? locate (источник, свер[0], старт) 
                                   : locatePattern (источник, свер, старт);
}
...
т_мера rindex(T, U=т_мера) (T[] источник, T[] свер, U старт=U.max)
{return rindex!(T)(источник, свер, старт);}

т_мера rindex(T) (T[] источник, T[] свер, т_мера старт=т_мера.max)
{
        return (свер.length is 1) ? locatePrior (источник, свер[0], старт) 
                                   : locatePatternPrior (источник, свер, старт);
}
...
т_мера locate(T, U=т_мера) (T[] источник, T свер, U старт=0)
{return locate!(T) (источник, свер, старт);}

т_мера locate(T) (T[] источник, T свер, т_мера старт=0)
{
...
т_мера locatePrior(T, U=т_мера) (T[] источник, T свер, U старт=U.max)
{return locatePrior!(T)(источник, свер, старт);}

т_мера locatePrior(T) (T[] источник, T свер, т_мера старт=т_мера.max)
{
        if (старт > источник.length)
            старт = источник.length;
...
т_мера locatePattern(T, U=т_мера) (T[] источник, T[] свер, U старт=0)
{return locatePattern!(T) (источник, свер, старт);}

т_мера locatePattern(T) (T[] источник, T[] свер, т_мера старт=0)
{
        т_мера    индкс;
        ...
        if (свер.length && протяженность <= источник.length)
            while (протяженность)
                   if ((индкс = индексУ (p, свер[0], протяженность)) is протяженность)
                      ...
                      if (matching (p+=индкс, свер.ptr, свер.length))
...
т_мера locatePatternPrior(T, U=т_мера) (T[] источник, T[] свер, U старт=U.max)
{return locatePatternPrior!(T)(источник, свер, старт);}

т_мера locatePatternPrior(T) (T[] источник, T[] свер, т_мера старт=т_мера.max)
{
        ...
        if (свер.length && свер.length <= длин)
            while (старт)
                  {
                  старт = locatePrior (источник, свер[0], старт);
                  if (старт is длин)
                      break;
                  else
                     if ((старт + свер.length) <= длин)
                          if (matching (источник.ptr+старт, свер.ptr, свер.length))
                              return старт;
                  }
...
DelimFruct!(T) разграничители(T) (T[] ист, T[] установи)
{
        DelimFruct!(T) элементы;
...
/******************************************************************************
        ...
        the установи of разграничители is ignored insопрe a пара of quotes. Note
...
/******************************************************************************
        ...
        Helper fruct for обходчик quotes(). A fruct is a low 
        impact mechanism for capturing контекст relating в_ an 
        opApply (conjunction of the names struct и foreach)
...
debug (UnitTest)
{
        ...
        ткст[] q;
        foreach (элемент; quotes ("1 'avcc   cc ' 3", " "))
                 q ~= элемент;
