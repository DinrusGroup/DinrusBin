...
extern(D)
{
    ...
    цел пуб(бцел х);
    alias пуб дайПервУстБит;
    цел пубр(бцел х);
    alias пубр найдиПервУстБит;
    цел тб(in бцел *х, бцел номбит);
    alias тб тестируйБит;
    цел тбз(бцел *х, бцел номбит);
    alias тбз тестируйЗаполниБит;
    цел тбп(бцел *х, бцел номбит);
    alias тбп тестируйИзмениБит;
    цел тбу(бцел *х, бцел номбит);
    alias тбу тестируйУстановиБит;
    бцел развербит(бцел б);
    alias развербит разверниБайт;
    ббайт чипортБб(бцел адр_порта);
    alias чипортБб читайПортБбайт;
    бкрат чипортБк(бцел адр_порта);
    alias чипортБк читайПортБкрат;
    бкрат чипортБц(бцел адр_порта);
    alias чипортБц читайПортБцел;
    ббайт пипортБб(бцел адр_порта, ббайт зап);
    alias пипортБб пишиПортБбайт;
    бкрат пипортБк(бцел адр_порта, бкрат зап);
    alias пипортБк пишиПортБкрат;
    бкрат пипортБц(бцел адр_порта, бцел зап);
    alias  пипортБц пишиПортБцел;
    цел члоустбит32( бцел x ); //alias _члоустбит32 члоустбит32;
    бкрат битсвоп( бцел x );
    alias битсвоп переставьБит ;
    ...
    бкрат дайАтрибутыФайла(ткст имяф);
    alias дайАтрибутыФайла дайАтрыФайла;
    бул файл_ли(ткст имяф); //alias _файл_ли файл_ли;
    бул папка_ли(ткст имяп); //alias _папка_ли папка_ли;
    проц сменипап(ткст имяп);
    alias сменипап перейди_в;
    проц сделайпап(ткст имяп);
    alias сделайпап сделайПапку;
    проц удалипап(ткст имяп);
    alias удалипап  удалиПапку;
    ткст дайтекпап();
    alias дайтекпап дайТекущуюПапку;
    ткст [] списпап(ткст имяп, ткст образец);
    alias списпап  списокПапки;
    ...
    ткст извлекиРасш(ткст пимя);
    alias извлекиРасш дайРасш;
