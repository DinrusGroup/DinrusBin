...
struct z_stream
{
    ...
    voidpf          opaque;    /* private data объект passed to zalloc and zfree */
...
/*
   The application must update next_in and avail_in when avail_in есть
   dropped to zero. It must update next_out and avail_out when avail_out
   есть dropped to zero. The application must initialize zalloc, zfree and
   ...
   zalloc must return Z_NULL if there is not enough memory for the объект.
...
/*
  ...
  should never be zero before the call. The application can используй the
  ...
  avail_in is zero after the call if enough output space есть been provided
  ...
  the compression algorithm in any manner.
    ...
    deflate() returns Z_OK if some progress есть been made (more input
  processed or more output produced), Z_STREAM_END if all input есть been
  consumed and all output есть been produced (only when flush is set to
...
/*
  ...
  The application can используй the uncompressed output when it wants, for
  ...
  uncompressed data from that block есть been written to strm->next_out.  The
...
/*
   ...
   which dictionary есть been used by the compressor. (The adler32 value
   ...
   inconsistent (for example if deflate есть already been called for this stream
...
/*
   ...
   The stream will keep the same compression level and any other атрибуты
...
/*
   ...
   gzip file" and give up.
      ...
      If deflateSetHeader is not used, the default gzip header есть text false,
...
/*
   ...
   ожидалось one (incorrect adler32 value). inflateSetDictionary does not
...
/*
   ...
   The stream will keep атрибуты that may have been set by inflateInit2.
...
int compress(Bytef*  dest,
             ...
             uLong   sourceLen);
/*
     Compresses the source buffer преобр_в the destination buffer.  sourceLen is
...
int compress2(Bytef*  dest,
              ...
              int     level);
/*
     Compresses the source buffer преобр_в the destination buffer. The level
   parameter есть the same meaning as in deflateInit.  sourceLen is the byte
...
int uncompress(Bytef*  dest,
               ...
               uLong   sourceLen);
/*
     Decompresses the source buffer преобр_в the destination buffer.  sourceLen is
   the byte length of the source buffer. Upon entry, destLen is the total
...
/*
   ...
   fileno (in the file есть been previously opened with fopen).
...
/*
   ...
   of bytes преобр_в the buffer.
     gzread returns the number of uncompressed bytes actually read (0 for
   end of file, -1 for error). */

int gzwrite(gzFile file, voidpc buf, uint len);
/*
     Writes the given number of uncompressed bytes преобр_в the compressed file.
...
/*
      Writes c, converted to an unsigned char, преобр_в the compressed file.
...
/*
   ...
   character есть been pushed but not read yet, or if c is -1. The pushed
...
/*
     Flushes all pending output преобр_в the compressed file. The parameter
...
/*
     Combine two Adler-32 checksums преобр_в one.  For two sequences of bytes, seq1
...
/*
     Combine two CRC-32 check values преобр_в one.  For two sequences of bytes,
