...
**
** Конвертировал Андерс Берг (Anders Bergh) <anders1@gmail.com>
**
*************************************************************************
**
** 2001 September 15
**
** Автор не утверждает авторского права на исходники.  Вместо
** легальной заметки, благословение:
**
**    Творите добро, но не зло.
**    Находите прощение себе и прощайте других.
**    Свободно разделяйте, не беря больше, чем Вы дали.
**
*************************************************************************
** В этом файле-заголовке определен интерфейс, который библиотека SQLite
** предоставляет клиентским программам.  Если какая-то функция Си, структура, тип данных,
** либо же определение константы отсутствует в данном файле, тогда это
** неопубликованный API SQLite: он может изменяться без предупреждения
** и не иметь ссылки у программ, использующих SQLite.
**
** Некоторые определения в данном файле помечены как
** "experimental".  Экспериментальные интерфейсы это, как правило, новые
** недавно добавленные в SQLite средства.  We do not anticipate changes 
** to experimental interfaces but reserve to make minor changes if
** experience from use "in the wild" suggest such changes are prudent.
**
** Официальная документация по API языка Си для SQLite извлечена
** из комментариев в данном файле.  Этот файл является авторитетным источником
** информации о том, как работают интерфейсы SQLite.
**
** Этот файл называется при конфигурационном управлении "sqlite.h.in".
** Данный makefile вносит небольшие изменения в данный файл (такое как вставка
** номера версии) и меняет его название на "sqlite3.h" как
** часть процесса построения.
*/
...
version (build) pragma(link, "rulada");
...
** version 3.1.1 or greater at compile время, programs may use the test 
...
** [sqlite3_busy_timeout()] to имя but three) that are methods on this
** object.
*/
struct sqlite3;
...
/*
** The тип for a callback function.
...
**
** This interface is used to do a one-время evaluatation of zero
** or more SQL statements.  UTF-8 text of the SQL statements to
** be evaluted is passed in as the секунда parameter.  The statements
...
** The particular return value depends on the тип of error. 
**
*/
int sqlite3_exec(
...
const SQLITE_MISMATCH =   20;   /* Data тип mismatch */
...
** over время.  Software that uses extended result codes should expect
** to see new result codes in future releases of SQLite.
** 
** The symbolic имя for an extended result code always contains a related
...
**
** SQLite uses one of the following integer values as the секунда
...
** [sqlite3_file_control()] interface.  The секунда "op" argument
...
** abstract тип for a mutex object.  The SQLite core never looks
...
** in the имя of the object stands for "virtual file system".
**
...
**
** The zName field holds the имя of the VFS module.  The имя must
...
** </ul>
**
** The file I/O implementation can use the object тип flags to
...
** size of the output буфер is also passed as a parameter to both 
** methods. If the output буфер is not large enough, SQLITE_CANTOPEN
...
**
** The секунда argument is a boolean value that turns extended result
...
** тип INTEGER PRIMARY KEY then that column is another an alias for the
** rowid.
**
...
** is the third argument to this routine.  The секунда argument to
...
** a секунда process is holding a reserved lock that it is trying
** to promote to an exclusive lock.  The first process cannot proceed
** because it is blocked by the секунда and the секунда process cannot
** proceed because it is blocked by the first.  If both processes
** invoke the busy handlers, neither will make any progress.  Therefore,
** SQLite returns [SQLITE_BUSY] for the first process, hoping that this
** will induce the first process to release its read lock and allow
** the секунда process to proceed.
**
...
** буфер supplied as the секунда parameter whose size is given by
** the first parameter.  Note that the order of the
** first two parameters is reversed from snprintf().  This is an
** historical accident that cannot be fixed without breaking
** backwards compatibility.  Note also that sqlite3_snprintf()
** returns a pointer to its буфер instead of the number of
** characters actually written into the буфер.  We admit that
** the number of characters written would be a more useful return
** value but we cannot change the implementation of sqlite3_snprintf()
** now without breaking compatibility.
**
** As long as the буфер size is greater than zero, sqlite3_snprintf()
** guarantees that the буфер is always zero-terminated.  The first
** parameter "n" is the total size of the буфер, including space for
...
**
** <blockquote><pre>
**  INSERT INTO table1 VALUES('It''s a happy день!')
** </pre></blockquote>
**
...
** </pre></blockquote>
**
** This секунда example is an SQL syntax error.  As a general rule you
...
** currently outstanding (malloced but not freed).  The секунда
...
** a single callback can be registered at a время.  Each call
...
** The секунда parameter to the callback is an integer 
...
** at a время.  Each call to sqlite3_set_authorizer overrides the
...
** секунда parameter to the callback is an integer code that specifies
...
** codes is used as the секунда parameter.  The 5th parameter to the
** authorizer callback is the имя of the database ("main", "temp", 
** etc.) if applicable.  The 6th parameter to the authorizer callback
** is the имя of the inner-most trigger or view that is responsible for
...
** where N is the секунда argument to this function. The progress callback
** itself is identified by the third argument to this function. The fourth
** argument to this function is a void pointer passed to the progress callback
** function each время it is invoked.
**
...
** not previously exist, an error is returned.  The секунда option opens
...
**
** The fourth parameter to sqlite3_open_v2() is the имя of the
...
** or [sqlite3_open16()].
** The секунда argument "zSql" is the statement to be compiled, encoded
...
** [SQLITE_ERROR] result code and you would have to make a секунда call to
...
** AAA is an alphanumeric identifier and VVV is a variable имя according
...
** its variants.  The секунда
** argument is the index of the parameter to be set.  The first parameter has
** an index of 1. When the same named parameter is used more than once, секунда
** and subsequent
...
** The NNN value must be between 1 and the compile-время
...
** if the same host parameter имя is used multiple times, each occurrance
...
**
** This routine returns a pointer to the имя of the n-th parameter in a 
** [sqlite3_stmt | prepared statement].
** Host parameters of the form ":AAA" or "@AAA" or "$VVV" have a имя
** which is the string ":AAA" or "@AAA" or "$VVV".  
** In other words, the initial ":" or "$" or "@"
** is included as part of the имя.
** Parameters of the form "?" or "?NNN" have no имя.
**
...
**
** This routine returns the index of a host parameter with the given имя.
** The имя must match exactly.  If no parameter with the given имя is 
...
**
** These routines return the имя assigned to a particular column
...
** The секунда parameter is the column number.  The left-most column is
** number 0.
**
...
** The имя of the database or table or column can be returned as
** either a UTF8 or UTF16 string.  The _database_ routines return
** the database имя, the _table_ routines return the table имя, and
** the origin_ routines return the column имя.
...
** the statement, where N is the секунда function argument.
**
** If the Nth column returned by the statement is an expression
** or subquery and is not a column value, then all of these functions
** return NULL. Otherwise, they return the 
** имя of the attached database, table and column that query result
...
** prepared statement and column at the same время then the results are
** undefined.
*/
...
** expression or subquery) then the declared тип of the table
...
**
** Then this routine would return the string "VARIANT" for the секунда
** result column (i==1), and a NULL pointer for the first result column
** (i==0).
**
** SQLite uses dynamic run-время typing.  So just because a column
** is declared to contain a particular тип does not mean that the
** data stored in that column is of the declared тип.  SQLite is
...
** [SQLITE_ROW] is returned each время a new row of data is ready
** for processing by the caller. The values may be accessed using
** the [sqlite3_column_int | column access functions].
** sqlite3_step() is called again to retrieve the next row of data.
** 
** [SQLITE_ERROR] means that a run-время error (such as a constraint
...
** more threads at the same moment in время.
**
...
** called on the [sqlite3_stmt | prepared statement] for the first время,
...
** the секунда argument is the index of the column for which information 
...
** [SQLITE_INTEGER | datatype code] for the initial data тип
** of the result column.  The returned value is one of [SQLITE_INTEGER],
** [SQLITE_FLOAT], [SQLITE_TEXT], [SQLITE_BLOB], or [SQLITE_NULL].  The value
** returned by sqlite3_column_type() is only meaningful if no тип
** conversions have occurred as described below.  After a тип conversion,
** the value returned by sqlite3_column_type() is undefined.  Future
** versions of SQLite may change the behavior of sqlite3_column_type()
** following a тип conversion.
**
...
** C programmers.
**
** Note that when тип conversions occur, pointers returned by prior
...
** not инвалидируй a prior pointer, though of course the content of the буфер
...
**
** The pointers returned are valid until a тип conversion occurs as
...
** difference only between the two is that the секунда parameter, the
** имя of the (scalar) function or aggregate, is encoded in UTF-8 for
...
**
** The секунда parameter is the имя of the SQL function to be created
** or redefined.
** The length of the имя is limited to 255 bytes, exclusive of the 
** zero-terminator.  Note that the имя length limit is in bytes, not
** characters.  Any attempt to create a function with a longer имя
...
** functions with the same имя but with either differing numbers of
...
** a structure for storing their state.  The first время this routine
** is called for a particular aggregate, a new structure of size nBytes
** is allocated, zeroed, and returned.  On subsequent calls (for the
** same aggregate instance) the same буфер is returned.  The implementation
** of the aggregate can use the returned буфер to accumulate data.
**
** The буфер allocated is freed automatically by SQLite whan the aggregate
** query concludes.
**
...
** call, where N is the секунда parameter. If no meta-data has been set for
...
** expressions that are constant at compile время. This includes literal
...
**
** The имя of the new collation sequence is specified as a UTF-8 string
** for sqlite3_create_collation() and sqlite3_create_collation_v2()
** and a UTF-16 string for sqlite3_create_collation16().  In all cases
** the имя is passed as the секунда function argument.
**
...
** sequence (so that SQLite cannot call it anymore). Each время the user
...
** the first string is less than, equal to, or greater than the секунда
...
** of the секунда argument to sqlite3_collation_needed() or
** sqlite3_collation_needed16(). The секунда argument is the database
** handle. The third argument is one of [SQLITE_UTF8], [SQLITE_UTF16BE], or
** [SQLITE_UTF16LE], indicating the most desirable form of the collation
** sequence function required. The fourth parameter is the имя of the
...
** millisecond время resolution, then the время will be rounded up to 
** the nearest секунда. The number of milliseconds of sleep actually 
...
** the имя of a folder (a.ka. directory), then all temporary files
...
**
** The секунда argument is a pointer to the function to invoke when a 
** row is updated, inserted or deleted. The first argument to the callback is
** a copy of the third argument to sqlite3_update_hook(). The секунда callback 
** argument is one of SQLITE_INSERT, SQLITE_DELETE or SQLITE_UPDATE, depending
** on the operation that caused the callback to be invoked. The third and 
** fourth arguments to the callback contain pointers to the database and 
** table имя containing the affected row. The final callback parameter is 
...
** in effect at the время they were opened.
**
...
** argument.
**
** The column is identified by the секунда, third and fourth parameters to 
** this function. The секунда parameter is either the имя of the database
...
** имя of the desired column, respectively. Neither of these parameters 
** may be NULL.
**
...
** declaration тип and collation sequence is valid only until the next 
...
int sqlite3_table_column_metadata(
  ...
  char* zDbName,              /* Database имя or NULL */
  char* zTableName,           /* Table имя */
  char* zColumnName,          /* Column имя */
  char** pzDataType,          /* OUTPUT: Declared data тип */
  char** pzCollSeq,           /* OUTPUT: Collation sequence имя */
...
** имя of the entry point defaults to "sqlite3_extension_init".
**
...
** If this is a problem for you, do not use the interface at this время.
**
...
/*
** This routine is used to register a new module имя with an SQLite
...
** имя and number of parameters exists.  If no such function exists
...
** [sqlite3_blob | blob-handle] into a caller supplied буфер.
** n bytes of data are copied into буфер
...
** [sqlite3_blob | blob-handle] from a user supplied буфер.
** n bytes of data are copied from the буфер
...
** имя.  Names are case sensitive.  If there is no match, a NULL
...
** same имя are registered, the behavior is undefined.  If a
** VFS is registered with a имя that is NULL or an empty string,
...
** is selected automatically at compile-время.  The following
...
** with a particular database identified by the секунда argument.  The
** имя of the database is the имя assigned to the database by the
** <a href="lang_attach.html">ATTACH</a> SQL command that opened the
** database.  To control the main database file, use the имя "main"
** or a NULL pointer.  The third and fourth parameters to this routine
** are passed directly through to the секунда and third parameters of
** the xFileControl method.  The return value of the xFileControl
** method becomes the return value of this routine.
**
** If the секунда parameter (zDbName) does not match the имя of any
