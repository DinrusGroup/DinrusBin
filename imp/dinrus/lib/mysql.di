...
/* NOTE (krox@gmx.net, May 2008): I've put a size_t everywhere, where a
"unsigned long" wasin the C-Header. Should work on Linux 32/64 bit, dont know
for other platforms. Remove this note when sombody checked. */
...
version (Windows) {
	...
	pragma (сооб, "You will need to manually link in the MySQL library.");
}
...
struct st_net {
 ...
 // ubyte *buff,*buff_end,*write_pos,*read_pos;
  ...
  size_t pkt_nr,compress_pkt_nr;
  size_t write_timeout, read_timeout, retry_count;
  int fcntl;
  my_bool compress;
  ...
  size_t remain_in_buf,length, buf_length, where_b;
  uint *return_status;
...
my_bool my_net_write(NET *net, char *packet,size_t len);
my_bool net_write_command(NET *net,ubyte command,
                           char *header, size_t head_len,
                           char *packet, size_t len);
int net_real_write(NET *net, char *packet,size_t len);
size_t my_net_read(NET *net);
...
struct sockaddr;
int my_connect(my_socket s,  sockaddr *имя, uint namelen,
               uint timeout);

struct rand_struct {
  size_t seed1,seed2,max_value;
...
void randominit(rand_struct *, size_t seed1,
                size_t seed2);
double my_rnd(rand_struct *);
void create_random_string(char *to, uint length, rand_struct *rand_st);

void hash_password(size_t *to,  char *password, uint password_len);
...
my_bool check_scramble_323( char *,  char *message,
                           size_t *salt);
void get_salt_from_password_323(size_t *res,  char *password);
void make_password_from_salt_323(char *to,  size_t *salt);
...
struct st_mysql_time {
  uint год, месяц, день, час, минута, секунда;
  size_t second_part;
  my_bool neg;
...
struct st_mysql_options {
  ...
  uint порт, protocol;
  size_t client_flag;
  //char *host,*user,*password,*unix_socket,*бд;
  char* host,user,password,unix_socket,бд;
  void *init_commands;	// actually, its st_dynamic_array*
...
struct st_mysql_parameters {
  size_t *p_max_allowed_packet;
  size_t *p_net_buffer_length;
};
...
MYSQL * mysql_real_connect(MYSQL *mysql,  char *host,
                                           ...
                                           uint порт,
                                            char *unix_socket,
                                           uint флаг_клиента);
int mysql_select_db(MYSQL *mysql,  char *бд);
...
int mysql_kill(MYSQL *mysql,size_t pid);
...
size_t mysql_get_client_version();
 char * mysql_get_host_info(MYSQL *mysql);
size_t mysql_get_server_version(MYSQL *mysql);
...
size_t * mysql_fetch_lengths(MYSQL_RES *result);
MYSQL_FIELD * mysql_fetch_field(MYSQL_RES *result);
MYSQL_RES * mysql_list_fields(MYSQL *mysql,  char *таблица,
                                           char *wild);
size_t mysql_escape_string(char *to, char *from,
                                            size_t from_length);
size_t mysql_hex_string(char *to, char *from,
                                         size_t from_length);
size_t mysql_real_escape_string(MYSQL *mysql,
                                               char *to, char *from,
                                               size_t length);
//void mysql_debug( char *debug);
void mysql_debug( char *debugB);   // debug a reserved word in D ?. I have renamed to *debugD

// I can't find this in the headers for mysql 5.1 -- is it mysql 5.0 only?
...
void myodbc_remove_escape(MYSQL *mysql,char *имя);
...
version(MySQL_51)
{
	struct st_mysql_bind
	{
	  ...
	  void		*буфер;	  /* буфер to get/put data */
	  ...
	  void (*skip_result)(st_mysql_bind *, MYSQL_FIELD *,
			      ubyte **row);
	  /* output буфер length, must be set when fetching str/binary */
	  size_t buffer_length;
	  size_t offset;           /* offset position for char/binary fetch */
	  size_t length_value;     /* Used if length is 0 */
	  uint	param_number;	  /* For null count and ошибка messages */
	  uint  pack_length;	  /* Internal length for packed data */
	  enum_field_types тип_буфера;	/* буфер тип */
	  my_bool       error_value;      /* used if ошибка is 0 */
	  my_bool       без_знака;      /* set if integer тип is unsigned */
...
struct st_mysql_stmt {
  ...
  MYSQL_ROWS *data_cursor;
  version(MySQL_51) {	 
	  int (*read_row_func)(st_mysql_stmt *инстр,
	                                  ubyte **row);
	  my_ulonglong affected_rows;
	  my_ulonglong insert_id;
  }
...
struct st_mysql_methods {
  ...
  void (*fetch_lengths)(size_t *to,
                        MYSQL_ROW column, uint field_count);
  void (*flush_use_result)(MYSQL *mysql);

  MYSQL_FIELD * (*list_fields)(MYSQL *mysql);
  my_bool (*read_prepare_result)(MYSQL *mysql, MYSQL_STMT *инстр);
  int (*stmt_execute)(MYSQL_STMT *инстр);
  int (*read_binary_rows)(MYSQL_STMT *инстр);
...
int mysql_stmt_prepare(MYSQL_STMT *инстр,  char *query,
                               size_t length);
int mysql_stmt_execute(MYSQL_STMT *инстр);
int mysql_stmt_fetch(MYSQL_STMT *инстр);
int mysql_stmt_fetch_column(MYSQL_STMT *инстр, MYSQL_BIND *bind,
                                    uint column,
                                    size_t offset);
int mysql_stmt_store_result(MYSQL_STMT *инстр);
size_t mysql_stmt_param_count(MYSQL_STMT * инстр);
my_bool mysql_stmt_attr_set(MYSQL_STMT *инстр,
                                    enum_stmt_attr_type attr_type,
                                     void *attr);
my_bool mysql_stmt_attr_get(MYSQL_STMT *инстр,
                                    enum_stmt_attr_type attr_type,
                                    void *attr);
my_bool mysql_stmt_bind_param(MYSQL_STMT * инстр, MYSQL_BIND * bnd);
my_bool mysql_stmt_bind_result(MYSQL_STMT * инстр, MYSQL_BIND * bnd);
my_bool mysql_stmt_close(MYSQL_STMT * инстр);
my_bool mysql_stmt_reset(MYSQL_STMT * инстр);
my_bool mysql_stmt_free_result(MYSQL_STMT *инстр);
my_bool mysql_stmt_send_long_data(MYSQL_STMT *инстр,
                                          uint param_number,
                                           char *data,
                                          size_t length);
MYSQL_RES * mysql_stmt_result_metadata(MYSQL_STMT *инстр);
MYSQL_RES * mysql_stmt_param_metadata(MYSQL_STMT *инстр);
uint mysql_stmt_errno(MYSQL_STMT * инстр);
 char * mysql_stmt_error(MYSQL_STMT * инстр);
 char * mysql_stmt_sqlstate(MYSQL_STMT * инстр);
MYSQL_ROW_OFFSET mysql_stmt_row_seek(MYSQL_STMT *инстр,
                                             MYSQL_ROW_OFFSET offset);
MYSQL_ROW_OFFSET mysql_stmt_row_tell(MYSQL_STMT *инстр);
void mysql_stmt_data_seek(MYSQL_STMT *инстр, my_ulonglong offset);
my_ulonglong mysql_stmt_num_rows(MYSQL_STMT *инстр);
my_ulonglong mysql_stmt_affected_rows(MYSQL_STMT *инстр);
my_ulonglong mysql_stmt_insert_id(MYSQL_STMT *инстр);
uint mysql_stmt_field_count(MYSQL_STMT *инстр);
...
enum : uint {
    ...
    CLIENT_CONNECT_WITH_DB = 8,    /* One can specify бд on connect */
    CLIENT_NO_SCHEMA = 16, /* Don't allow database.таблица.column */
    ...
    CLIENT_MULTI_STATEMENTS = (1 << 16), /* Enable/disable multi-инстр support */
    CLIENT_MULTI_RESULTS = (1 << 17), /* Enable/disable multi-результаты */
