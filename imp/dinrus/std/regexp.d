...
alias RegExpException ИсклРегВыр;
alias sub подставь;
alias find найди;
...
{   int srcsave;
	...
	switch (program[pc])
	{
		...
		if (c1 != c2)
		{
			...
			c2 = cidrus.toupper(cast(rchar)c2);
		    else
			goto Lnomatch;
		    if (c1 != c2)
			goto Lnomatch;
...
{   rchar c;
	...
	switch (prog[i])
	{
		...
		if (c <= 0x7F)
		{   r.setbit2(c);
		    r.setbit2(cidrus.tolower(cast(rchar)c));
		}
		return 1;
		...
		if (c <= 0x7F)
		{   r.setbit2(cidrus.toupper(cast(rchar)c));
		    r.setbit2(cidrus.tolower(cast(rchar)c));
		}
		return 1;
