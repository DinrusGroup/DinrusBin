...
/+
//Это необходимая реализация-обёртка для русскоязычного эквивалента
// предыдущего интерфейса
abstract class ПотокВвода:InputStream {
   ...
   * Throws a ReadException if it is not correct.
   */
  проц читайРовно(проц* буфер, т_мера разм)
  {
  readExact(cast(void *)буфер,cast(size_t) разм);
  }
  ...
  {
  return cast(т_мера)(read(cast(ubyte)буфер));
  }
  ...
  /***
   * Прочесть строку, оканчивающуюся какой-либо комбинацией из
   * возврата каретки, запитки строки или конца файла.
   *
   * Окончания не включаются. Версия с wchar аналогична.
   * Заполняется указанный в необязательном параметре буфер
   * (который при необходимость реаллоцируется), возвращается
   * срез с результатом. 
   */
  сим[] читайСтроку{ return cast(сим[])(readLine());}  
  сим[] читайСтроку(сим[] рез){return cast(сим[])(readLine(cast(char[]) рез));}
  шим[] читайШСтроку(){return cast(шим[])(readLineW());}			/// ditto
  шим[] читайШСтроку(шим[] рез){return cast(шим[])(readLine(cast(wchar[] рез));}	/// ditto

  /***
   * перегрузите инструкции foreach чтобы считывать поток строка за строкой
	* и вызовите предоставляемого делегата с каждой строкой илм с каждой
	* строкой и её номером.
   *
   * Фраза, передаваемая строкой, может использоваться повторно
   * между вызовами делегата.
  ...
  // проход по потоку строка за строкой
  int opApply(int delegate(inout char[] line) dg);
  int opApply(int delegate(inout ulong n, inout char[] line) dg);  /// ditto
  int opApply(int delegate(inout wchar[] line) dg);		   /// ditto
  int opApply(int delegate(inout ulong n, inout wchar[] line) dg); /// ditto
  ...
  /// Read a string of the given length,
  /// throwing ReadException if there was a problem.
  сим[] читайТекст(т_мера длина)
  {return cast(сим)(readString(cast(size_t) длина));}
  ...
  шим[] читайШТекст(т_мера длина)
  {return cast(шим)(readStringW(cast(size_t) длина));}


  /***
   * Прочесть и вернуть следующий символ потока.
   *
   * Это единственный метод, который будет правильно обрабатывать ungetc.
   * Формат getcw зависим от реализации.
   * Если достигается EOF, то getc возвращает char.init, а getcw - wchar.init.
   */

  сим берис(){ return cast(сим)getc();}
  шим беришс(){ return cast(шим)getcw();} /// ditto

  /***
   * Поместить символ обратно в поток.
   *
   * They will be returned in first-in last-out order from getc/getcw.
   * Only has effect on further calls to getc() and getcw().
   */
  сим сброс(сим с){return cast(сим)(ungetc(cast(char) c));}
  шим сброшс(шим с){ return cast(шим)(ungetcw(cast(wchar) c));} /// ditto
   ...
   * --------------------------
   */
  цел вчитайф(ИнфОТипе[] аргументы, проц* арги)
  {
  auto arguments = аргументы;
  auto args = арги;  
  return cast(цел)(vreadf(cast(TypeInfo[]) arguments, cast(void*) args));
  }
  
  int readf(...); /// TODO : здесь требуется повозиться!!!!

  /// Получить инфу о числе доступных для чтения байтов.
  т_мера сколько_есть(){ return cast(т_мера) available();}
  ...
  бул кф_ли(){ return cast(бул)(eof());}

  бул открыт_ли()( return cast(бул) isOpen());}	/// Return true if the stream is currently open.
}
+/
...
/// Interface for writable streams.
abstract class ПотокВывода:OutputStream {

  /***
   * Write exactly size bytes from buffer, or throw a WriteException if that
   * could not be done.
   */
  проц пишиРовно(проц* буфер, т_мера разм){ writeExact(cast(void*) буфер, cast(size_t) разм);}

  /***
   * Write as much of the buffer as possible,
   * returning the number of bytes written.
   */
  т_мера пиши(ббайт[] буфер){return cast(т_мера) (write(cast(ubyte[]) буфер));}
   ...
   * Throws WriteException on error.
   */
  проц пишиСтроку(сим[] s){writeLine(cast(char[]) s);}
   ...
   * Throws WriteException on error.
   */
  проц пишиШСтроку(шим[] s){writeLineW(cast(wchar[]) s);}
   ...
   * Throws WriteException if it could not be fully written.
   */
  проц пишиТекст(char[] s){writeString(cast(char[]) s);}
   ...
   * Throws WriteException if it could not be fully written.
   */
  void пишиШТекст(шим[] s){writeStringW(cast(wchar[]) s);}

  /***
   * Print a formatted string into the stream using printf-style syntax,
   * returning the number of bytes written.
   */
  т_мера ввыводф(сим[] формат, спис_ва арги)
  {
  auto format = формат;
  auto args = арги;
  return cast(т_мера) vprintf(cast(char[]) format, cast(va_list) args);
  }
  
  size_t printf(char[] format, ...);	/// TODO
...
class Stream : InputStream, OutputStream {
  private import std.string, crc32, cidrus, cidrus;
...
alias handle хэндл;
  ...
  void open(string filename, FileMode mode = FileMode.In) {
    ...
    version (Posix) {
      hFile = os.posix.open(toStringz(filename), access | createMode, share);
      isopen = hFile != -1;
    }
    if (!isopen)
  ...
  override size_t readBlock(void* buffer, size_t size) {
    ...
    version (Win32) {
      ...
      size = os.posix.read(hFile, buffer, size);
      if (size == -1)
	size = 0;
    }
    readEOF = (size == 0);
  ...
  override size_t writeBlock(void* buffer, size_t size) {
    assertWriteable();
    version (Win32) {
      winapi.WriteFile(hFile, buffer, size, &size, null);
    } else version (Posix) {
      size = os.posix.write(hFile, buffer, size);
      if (size == -1)
	size = 0;
    }
    return size;
